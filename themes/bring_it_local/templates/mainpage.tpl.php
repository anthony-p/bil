...
?>
	...
	<div class="featuredAuctions">
		<?php $cols = min(count($magento_items),$layout['hpfeat_nb']);?>
			<div class="row">
			<? for ($i=0; $i<$cols; $i++) { ?>
			<div class="block">
				<a href="<?php echo $coupon_url;?>/<?=$magento_items[$i]['url'];?>" class="image"><img src="<?=$magento_items[$i]['image_url'];?>" border="0" width="125" alt="<?=$magento_items[$i]['name'];?>"></a>
				<a href="<?php echo $coupon_url;?>/<?=$magento_items[$i]['url'];?>" class="link"><?=title_resize($magento_items[$i]['name']);?></a>
				</div>
				 <? } ?>
			<div class="clear"></div>
		</div>
 ...
 -->
		...
		<table border="0" cellpadding="0" cellspacing="0" id="recentWantedAds" class="mainTable">
			...
			<tr height="15" class="<?=$background;?>">
				...
				<td><a href="<?=process_link('wanted_details', array('name' => $item_details['name'], 'wanted_ad_id' => $item_details['wanted_ad_id']));?>"><img src="themes/<?=$setts['default_theme'];?>/img/wanted.gif" width="13" height="12" hspace="3" border="0"></a></td> 
			</tr> 
			<? } ?>
		</table>
		<? } ?>
