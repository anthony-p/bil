...
foreach ($setts['banner_positions'] as $key => $value)
{
	...
	{
		#$banner_position[$key] = '<div align="center" style="padding-top: 3px; padding-bottom: 3px;">banner here-np/npglobal_header.php</div>';
$banner_position[$key] = '';
	}
}
...
#	$template->set('member_username', $session->value('username'));
	...
	$menu_box_content	.= $template->process('npheader_members_menu.tpl.php');
	$template->set('menu_box_content', $menu_box_content);
}
...
if ($setts['enable_header_counter'] && eregi('index.php', $_SERVER['PHP_SELF']))
{
	...
	$template->set('nb_site_users', $db->count_nprows('users', "WHERE active='1'"));
...
$template_output .= $template->process('npheader.tpl.php');
