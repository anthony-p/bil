...
// send data
function ajaxUpload(form,url,div_id,show_loading,html_error){
...
function ajaxSend() {
	...
	if ((!title) || (!email) || (!description) || (!uploadfile) || (!code)) {
	document.getElementById('info_message').innerHTML = '<div class="wrong">You must to fill all fields!</div>';
	refreshCaptcha();
	} else if (!validate(email)) { document.getElementById('info_message').innerHTML = '<div class="wrong">Email address is not valid!</div>';
	} else {
	ajaxRequest(title, email, description, uploadfile, uploadedfile, code); }
	
	return false;
}
function ajaxRequest(title, email, description, uploadfile, uploadedfile, code)
{
	document.getElementById("info_message").innerHTML="<img src='img/loading.gif' />";
	var url="insert.php?do=insert&title="+title+"&email="+email+"&description="+description+"&uploadfile="+uploadfile+"&uploadedfile="+uploadedfile+"&code="+code;
	...
	xmlhttp.onreadystatechange=function()
	{
		if (xmlhttp.readyState==4)
			document.getElementById("info_message").innerHTML=xmlhttp.responseText;
			formDone();
	}
