...
if (INDEX_PAGE == 1 && !$_SESSION['index_page_viewed'])
{
	if ($integration['enable_integration'] && $integration['main_page_unified'])
	{
		...
		header_redirect($integration['ppb_path'] . 'npindex_unified.php');
	}
}
...
function pps_convert_username($username)
{
	...
	{
		$counter++;
		$is_username = $db->count_nprows('users', "WHERE username='" . $new_username . "'");
...
if ($is_deleted_pps)
{
	...
	include_once ($fileExtension.'includes/npclass_item.php');
	include_once ($fileExtension.'includes/npclass_user.php');
...
if (
		...
		!eregi('npaccount_activate.php', $_SERVER['PHP_SELF'])
	)
{
