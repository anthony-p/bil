...
include_once ('npclass_formchecker.php');

class npcustom_field extends npformchecker
{
	var $vars = array();
	var $show_only = false;
	var $data_owner_id = 0;
	...
	function display_box($box_id, $box_name, $box_type, $box_value, $selected_value = NULL)
	{
		...
		{
			case 'text':
				$display_output .= '<input type="text" name="custom_box_' . $box_id . '" value="' . ((!empty($selected_value)) ? $selected_value : $box_value) . '" /> ';
				break;
			case 'textarea':
	...
	function display_fields($section_id, $page_handle, $category_id = 0)
	{
		...
		if ($this->box_search)
		{
			$addl_query = ($category_id) ? " AND category_id IN (0, " . intval($category_id) . ")" : " AND category_id=0";
		}
		else 
		{
			$addl_query = ($category_id) ? " AND category_id IN (0, " . intval($category_id) . ")" : '';			
		}
