...
define('GMSG_MAIL_FROM_ADMIN', 'Site Administrator');
...
function block_reason ($user_id, $owner_id, $block_type = 'bid')
{
	...
	$block_details = $db->get_sql_row("SELECT b.*, u.username FROM " . DB_PREFIX . "blocked_users b
		LEFT JOIN " . NPDB_PREFIX . "users u ON u.user_id=b.user_id WHERE
...
function meta_tags ($base_url, $parent_id, $auction_id, $wanted_ad_id, $shop_id)
{
	...
	else
	{
		...
		{
			$user_details = $db->get_sql_row("SELECT shop_name, shop_metatags FROM " . NPDB_PREFIX . "users WHERE 
...
function user_pics ($user_id, $reputation_only = false, $reverse = false)
{
	...
	$user_details = $db->get_sql_row("SELECT enable_aboutme_page, shop_active, seller_verified, enable_profile_page FROM " . NPDB_PREFIX . "users WHERE user_id='" . $user_id . "'");
...
function user_account_management($user_id, $active)
{
	...
	$db->query("UPDATE " . NPDB_PREFIX . "users SET active=" . $active . ", payment_status='confirmed' WHERE user_id='" . $user_id . "'");
...
function suspend_debit_users()
{
	...
	{
		$sql_select_users = $db->query("SELECT user_id FROM " . NPDB_PREFIX . "users WHERE 
			active=1 AND " . $query);
...
function force_payment_enabled($user_id, $item_details)
{
	...
	$user_details = $db->get_sql_row("SELECT enable_force_payment FROM " . NPDB_PREFIX . "users WHERE 
...
function revert_sale($winner_details)
{
	...
	if ($winner_details['sale_fee_payer_id'])
	{
		...
		if ($payment_mode == 2)
		{
			$db->query("UPDATE " . NPDB_PREFIX . "users SET balance=balance-" . $winner_details['sale_fee_amount'] . " WHERE user_id='" . $winner_details['sale_fee_payer_id'] . "'");
		}
	}
 ...
 * @return array $totals
 */
function calculate_postage($winner_ids, $seller_id, $auction_id = null, $buyer_id = null, $buyer_country = null, $buyer_state = null, $sc_quantity = 0)
{
	global $db;
	...
	$user_details = $db->get_sql_row("SELECT * FROM " . NPDB_PREFIX . "users WHERE user_id='" . $seller_id . "'");
	...
	{
		$buyer_details = $db->get_sql_row("SELECT * FROM " . NPDB_PREFIX . "users WHERE user_id='" . $buyer_id . "'");
...
function shipping_locations($user_id)
{
	...
	$pc_shipping_locations = $db->get_sql_field("SELECT pc_shipping_locations FROM " . NPDB_PREFIX . "users WHERE 
