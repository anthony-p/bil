...
class nptax extends npfees_main
{
	...
	function countries_dropdown($select_name, $selected_value, $form_refresh = null, $form_refresh_type = 'register', $add_null_field = false, $null_field_caption = null)
	{
		(string) $display_output = null;
	...
	function apply_tax($fee_amount, $currency, $user_id, $enable_tax)
	{
		...
		if ($enable_tax)
		{
			$user_row = $this->get_sql_row("SELECT tax_exempted, country, state FROM " . NPDB_PREFIX . "users WHERE
	...
	function can_add_tax($user_id, $enable_tax)
	{
		...
		if ($enable_tax)
		{
			$user_row = $this->get_sql_row("SELECT tax_account_type, tax_reg_number, country, state FROM
				" . NPDB_PREFIX . "users WHERE user_id=" . intval($user_id));
	...
	function auction_tax($seller_id, $enable_tax, $buyer_id=0)
	{
		...
		if ($can_add_tax['can_add_tax'])
		{
			...
			if ($buyer_id)
			{
				$buyer_row = $this->get_sql_row("SELECT tax_exempted, country, state FROM " . NPDB_PREFIX . "users WHERE
