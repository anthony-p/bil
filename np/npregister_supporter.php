...
else
{
	...
	else
	{
		...
		if ($_REQUEST['operation'] == 'submit')
		{
			...
			include ('includes/npprocedure_frmchk_supporter.php'); /* Formchecker for user creation/edit */
			...
			else
			{
				...
				$template->set('register_success_header', header5(MSG_SUBMISSION_CONFIRMATION));## PHP Pro Bid v6.00 add signup fee procedure here.
				...
				else if ($setts['signup_settings'] == 1)
				{
					...
					$register_success_message = '<p align="center" class="contentfont">' . MSG_REGISTER_SUCCESS_TYPE3 . '</p>';## PHP Pro Bid v6.00 include registration confirmation email
					$mail_input_id = $user_id;
					include('language/' . $setts['site_lang'] . '/mails/npregister_confirm_user_recommended_notification.php');					
				}
				else if ($setts['signup_settings'] == 2)
				{
					...
					$register_success_message = '<p align="center" class="contentfont">' . MSG_REGISTER_SUCCESS_TYPE3 . '</p>';## PHP Pro Bid v6.00 notify user & admin that user approval is required
					$mail_input_id = $user_id;
				...
				else
				{
					...
					$register_success_message = '<p align="center" class="contentfont">' . MSG_REGISTER_SUCCESS_TYPE3 . '</p>';## PHP Pro Bid v6.00 include registration success email
					$mail_input_id = $user_id;
		...
		if (!$form_submitted)
		{
			$template->set('register_post_url', 'npregister_supporter.php');
...
#			$template->set('registration_terms_box', terms_box('registration', $_POST['agree_terms']));
			...
			$template_output .= $template->process('npregister_supporter.tpl.php');
		}
	}
