...
#Coupons (magento) recent deals
    ...
    $coupons_recent_deals_header = header1(MSG_BAR_FEATURED_AUCTIONS . ' <span class="viewAll"><a href="">' . MSG_VIEW_ALL . '</a></span>');
    $magento_count = 3;
    $template->set('coupons_recent_deals_header', $coupons_recent_deals_header);
    $template->set('magento_count', $magento_count);
...
if ($globalads=="1")## featured ads
{
	...
	$global_item_details = $db->random_rows('partners', 'advert_id, name, description, advert_code, currency, end_time', $select_condition, $layout['hpfeat_max']);
