...
if ($probid_user['probid_user_id']) {
	...
	$featured_auctions_user_header = header1('<span class="main-title">Featured Auctions from ' . $probid_user['username'] . '</span> <span class="viewAll"><a href="' . process_link('auctions_show', array('option' => 'localuser', 'user' => $probid_user['probid_user_id'])) . '">' . MSG_VIEW_ALL . '</a></span>');
...
$menuTemplate -> set('campaign_id', $np_userid);
...
$menuTemplate -> set("page_selected", $page_nr);
$menuTemplate -> set("total_pages", $total_pages);
