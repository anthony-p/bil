...
else
{
	...
	$invoices_array = array('product_invoice', 'auction_invoice', 'wanted_ad_invoice', 'fee_invoice', 'sc_invoice');
	...
	switch ($_REQUEST['invoice_type'])
	{
			...
			{				
				...
				while ($invoice_details = $db->fetch_array($sql_select_products))
				{
					if (!$single_settings) /* some page settings will only be taken once */
					{
						...
						$template->set('invoice_header', '<img src="images/bringitlocalogo.gif" border="0">');
						...
						$template->set('invoice_number', 'PR-' . intval($_REQUEST['invoice_id']));
	
						$seller_details = $db->get_sql_row("SELECT u.name, u.address, u.city, u.zip_code, u.tax_account_type, u.tax_company_name, 
		...
		case 'sc_invoice':
			...
			$sql_select_products = $db->query("SELECT sc.*, w.purchase_date, w.bid_amount, w.quantity_offered, a.name, a.auction_id FROM 
				" . DB_PREFIX . "shopping_carts sc 
				LEFT JOIN " . DB_PREFIX . "winners w ON w.sc_id=sc.sc_id
				LEFT JOIN " . DB_PREFIX . "auctions a ON a.auction_id=w.auction_id WHERE sc.sc_id='" . intval($_REQUEST['invoice_id']) . "' AND
				(sc.seller_id='" . $session->value('user_id') . "' OR sc.buyer_id='" . $session->value('user_id') . "')");
			...
			{				
				...
				while ($invoice_details = $db->fetch_array($sql_select_products))
				{
					if (!$single_settings) /* some page settings will only be taken once */
					{
						...
						$template->set('invoice_date', show_date($invoice_details['purchase_date'], false));
						$template->set('invoice_number', 'SCP-' . $invoice_id);
	
						$seller_details = $db->get_sql_row("SELECT u.name, u.address, u.city, u.zip_code, u.tax_account_type, u.tax_company_name, 
							c.name AS country_name, s.name AS state_name, u.state, u.shop_add_tax FROM " . DB_PREFIX ."users u
							LEFT JOIN " . DB_PREFIX . "countries s ON u.state=s.id
							LEFT JOIN " . DB_PREFIX . "countries c ON u.country=c.id WHERE u.user_id=" . $invoice_details['seller_id']);
						...
						$sc_postage = $invoice_details['sc_postage'];
						$sc_insurance = $invoice_details['sc_insurance'];
						
						/* tax applies to the whole cart */						
						$ad_tax = $tax->auction_tax($invoice_details['seller_id'], $setts['enable_tax'], $invoice_details['buyer_id']);
						
						$tax_rate = $invoice_details['tax_rate'];
	
						$tax_details = array(
							'apply' => (($tax_rate > 0) ? true : false),
							'tax_reg_number' => (($tax_rate > 0) ? $ad_tax['tax_reg_number'] : '-'),
							'tax_rate' => (($tax_rate > 0) ? $tax_rate . '%' : '-')
						);
					...
					$product_tax = ($tax_rate > 0) ? $product_no_tax * $tax_rate / 100 : 0;
					$total_tax += $product_tax;
				...
				$pi_no_tax = $sc_insurance + $sc_postage;
				$total_no_tax;
				
				$pi_tax = ($tax_rate > 0) ? $pi_no_tax * $tax_rate / 100 : 0;
				$total_tax += $pi_tax;
				
				$total_amount = $total_no_tax + $pi_no_tax + $total_tax;
	            ...
	            $template->set('total_postage', $fees->display_amount($sc_postage, $invoice_details['currency']));
	            $template->set('total_insurance', $fees->display_amount($sc_insurance, $invoice_details['currency']));
