...
//		u.user_id='" . $mail_input_id . "'");
    ...
    $row_details = $db->get_sql_row("SELECT u.id, u.first_name, u.last_name, u.email FROM  bl2_users u WHERE
		u.id='" . $mail_input_id . "'");
}
...
$activation_link = SITE_PATH . 'account_activate.php?user_id=' . $row_details['id'] . '&email=' . $row_details['email'];

$text_message = sprintf($text_message, $row_details['first_name']." ".$row_details['last_name'], $setts['sitename'], $row_details['email'], $activation_link);
$html_message = sprintf($html_message, $row_details['first_name']." ".$row_details['last_name'], $setts['sitename'], $row_details['email'], $activation_link);
