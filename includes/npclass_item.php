...
class item extends npcustom_field
{
	...
	function auction_approval ($item_details, $user_id)
	{
		...
		else
		{
			...
			$user_auction_approval = $this->get_sql_field("SELECT auction_approval FROM " . NPDB_PREFIX . "users WHERE
	...
	function relist_wanted_ad($wanted_ad_id, $user_id, $duration = 0, $charge_fees=true)
	{
		...
		$user_details = $this->get_sql_row("SELECT user_id, username, shop_account_id, shop_categories,
			...
			" . NPDB_PREFIX . "users WHERE user_id=" . $relist_details['owner_id']);
	...
	function rollback_transaction($auction_id, $user_id, $reverse_id = 0)
	{
		...
		$sql_update_balance = $this->query("UPDATE " . NPDB_PREFIX . "users SET balance='" . $rollback_details['balance'] . "' WHERE
			user_id=" . $user_id);
	}
	...
	function upload_manager($variables_array, $media_type = 1, $form_name = null, $manager_display = false, $new_table = false, $box_title = true, $display_fee = null, $reverse_dd = false)
	{
		...
		else
		{
   		...
   		$display_output .= '<tr class="c1"> '.
      		...
      		'	<td ' . (($this->upload_url) ? (($media_type == 2 && $this->setts['enable_embedded_media']) ? 'rowspan="3"' : 'rowspan="2"') : '') . ' width="100%"><input type="button" name="btn_upload_file" value="' . GMSG_UPLOAD_IMAGE . '" onclick="submit_form(' . $form_name . ', ' . $media_type . ');" ' . $submit_btn_status . ' /></td> '.
   			'</tr> ';
   		...
   		if ($nb_uploads)
   		{
			   ...
			   $rows = floor($nb_uploads/$columns) + 1;
      		   ...
      		   for ($j=0; $j<$columns; $j++) 
		   		{
		   			...
		   			$display_output .= '<td width="' . $width . '" style="padding-right: 4px; padding-bottom: 4px;"> ';

		   			if (!empty($variables_array[$media_name][$counter]))
		   			{
		   				$display_output .= '<table border="0" cellspacing="1" cellpadding="2" class="border c2"> '.
			   				'	<tr> '.
			   				'		<td align="center"> ';

			   				if ($media_type == 1) /* image thumbnail */
			   				{
			   					$display_output .= '<img src="' . SITE_PATH . 'thumbnail.php?pic=' . $variables_array[$media_name][$counter] . '&w=80&sq=Y&b=Y" border="0" alt="' . $variables_array[$media_name][$counter] . '"> ';
			   				}
			   				else if ($media_type == 2) /* video thumbnail */
			   				{
			   					$display_output .= '<img src="' . SITE_PATH . 'thumbnail.php?pic=images/media_icon.gif&w=80&sq=Y&b=Y" border="0" alt="' . $variables_array[$media_name][$counter] . '"> ';
			   				}
			   				else if ($media_type == 3) /* digital media thumbnail */
			   				{
			   					$display_output .= '<img src="' . SITE_PATH . 'thumbnail.php?pic=images/dd_icon.gif&w=80&sq=Y&b=Y" border="0" alt="' . $this->addlfile_name_display($variables_array[$media_name][$counter], $variables_array['reverse_id']) . '"> ';
			   				}
			   				...
			   				$display_output .= '</td> '.
				   				...
				   				'			<tr> '.
				   				'				<td>' . GMSG_DELETE . '</td> '.
				   				'				<td><input type="checkbox" name="delete_media[]" value="1" onclick="delete_media(' . $form_name . ', ' . $media_type . ', ' . $counter . ');" /></td> '.
				   				'			</tr> '.
				   				'		</table></td> '.
				   				'	</tr> '.
				   				'</table>';
	...
	function select_direct_payment($selected_values, $user_id, $show_only = false, $text_only = false, $value_only = false)
	{
		(array) $output = null;
		...
		while ($payment_gateway = $this->fetch_array($sql_select_gateways))
		{
			...
			else
			{
				...
				if (!$show_only)
				{
					$user_details = $this->get_sql_row("SELECT * FROM " . NPDB_PREFIX . "users WHERE user_id=" . intval($user_id));
	...
	function under_time($item_details)
	{
		...
		return ($time_left>CURRENT_TIME || $item_details['auction_type'] == 'first_bidder') ? false : true;
	}
	...
	function can_close_manually($item_details, $user_id)
	{
		...
		return ($this->setts['end_auction_early'] || 
	...
	function update_prefilled($prefilled_fields, $user_id)
	{
		...
		$sql_update_prefilled = $this->query("UPDATE " . NPDB_PREFIX . "users SET
	...
	function update_mailprefs($mail_prefs, $user_id)
	{
		...
		$sql_update_mailprefs= $this->query("UPDATE " . NPDB_PREFIX . "users SET
	...
	function relist($auction_id, $user_id, $duration = 0, $charge_fees=true, $auto_relist = false)
	{
		...
		$item_details = $this->get_sql_row("SELECT a.*, u.shop_account_id, u.shop_active, u.seller_verified, 
			...
			LEFT JOIN " . NPDB_PREFIX . "users u ON a.owner_id=u.user_id
		...
		if ($can_list)
		{
			...
			if ($relist_option == 1)
			{
				$sql_relist_auction = $this->query("INSERT INTO " . DB_PREFIX . "auctions
					...
					fb_decrement_amount, fb_decrement_interval)
					SELECT
					name, description, quantity, auction_type, start_price, reserve_price, buyout_price, bid_increment_amount,
					country, zip_code, shipping_method, shipping_int, payment_methods, category_id, owner_id, hpfeat, catfeat,
					bold, hl, hidden_bidding, currency, postage_amount, insurance_amount, type_service, enable_swap,
			...
			$relist_field[] = "end_time = '" . (($relist_details['auction_type'] == 'first_bidder') ? 0 : $auction_end_time) . "'";
			$relist_field[] = "duration = '" . $auction_duration . "'";
			$relist_field[] = "closed = '0'";
			$relist_field[] = "nb_bids = '0'";
	...
	function prepare_reputation($winning_bid_details, $item_details, $reverse_id = 0)
	{
		...
		if (!$reverse_id)
		{
			$this->query("UPDATE " . NPDB_PREFIX . "users SET items_sold=items_sold+1 WHERE user_id='" . $winning_bid_details['seller_id'] . "'");
			$this->query("UPDATE " . NPDB_PREFIX . "users SET items_bought=items_bought+1 WHERE user_id='" . $winning_bid_details['buyer_id'] . "'");
		}
	...
	function assign_winner($item_details, $sale_type = 'high_bid', $buyer_id = 0, $purchased_quantity = 0, $offer_details = null, $force_payment = 0)
	{
	...
	function send_invoice($variables_array, $user_id, $total_postage = null, $sender_id = null)
	{
		...
		if ($invoice_id > 0)
		{
			...
			$seller_details = $this->get_sql_row("SELECT * FROM " . NPDB_PREFIX . "users WHERE 
				user_id='" . $user_id . "'");
			...
			{
				$total_postage = calculate_postage($variables_array['winner_id'], $seller_details['user_id']);				
			...
			for ($i=0; $i<$nb_products; $i++)
			{
				...
				$this->query("UPDATE " . DB_PREFIX . "winners SET invoice_sent='1', 
					...
					invoice_id='" . $invoice_id . "' WHERE winner_id='" . $winner_id . "' AND " . $user_type . "='" . $sender_id . "'");
	...
	function show_high_bid($item_details, $output_type = 'high_bid')
	{
		...
		if ($output_type == 'winner')
		{
			$sql_select_winners = $this->query("SELECT w.bid_amount, w.quantity_offered, w.buyout_purchase, u.username, u.user_id FROM " . DB_PREFIX . "winners w
				LEFT JOIN " . NPDB_PREFIX . "users u ON w.buyer_id=u.user_id WHERE
		...
		if ($output_type == 'high_bid')
		{
			if ($item_details['nb_bids'])
			{
				...
				$sql_select_bids = $this->query("SELECT b.bid_amount, u.username, u.user_id FROM " . DB_PREFIX . "bids b
					LEFT JOIN " . NPDB_PREFIX . "users u ON b.bidder_id=u.user_id WHERE
	...
	function winners_message_board_link($item_details, $user_id)
	{
		...
		if ($is_winner)
		{
			$sql_select_wins = $this->query("SELECT w.*, b.username AS buyer_username, s.username AS seller_username FROM " . DB_PREFIX . "winners w
				LEFT JOIN " . NPDB_PREFIX . "users b ON w.buyer_id=b.user_id
				LEFT JOIN " . NPDB_PREFIX . "users s ON w.seller_id=s.user_id	WHERE
	...
	function direct_payment_multiple_box ($invoice_id, $items_array, $dp_array, $buyer_id)
	{
		...
		if ($this->direct_payment_multiple($invoice_id, $items_array, $dp_array, $buyer_id)) 
		{
			...
			foreach ($items_array as $item_details)
			{
				...
				$total_insurance += ($item_details['insurance_included']) ? $insurance_amount : 0;
	...
	function offer_options($auction_id, $offer_id, $offer_accepted, $can_make_offer, $offer_type = 'auction_offers')
	{
		...
		{
			$display_output .= '[ <a href="npmembers_area.php?page=selling&section=view_offers&do=accept_offer&offer_id=' . $offer_id .
				'&offer_type=' . $offer_type . '&auction_id=' . $auction_id . '" onclick="return confirm(\'' . MSG_ACCEPT_OFFER_CONFIRM . '\');">' . MSG_ACCEPT_OFFER . '</a> ] ' ;
		}

		if ($offer_type != 'bids')
		{
			$display_output .= '[ <a href="npmembers_area.php?page=selling&section=view_offers&do=delete_offer&offer_id=' . $offer_id .
		...
		if ($invoice_details['invoice_id'] && (!$invoice_details['refund_request'] || $in_admin))
		{
			...
			switch ($refund_request)
			{
				...
				case 2: // request approved - made by admin
					...
					$user_details = $this->get_sql_row("SELECT * FROM " . NPDB_PREFIX . "users WHERE user_id='" . $invoice_details['user_id'] . "'");
					
					if ($this->fees->user_payment_mode($user_details['user_id']) == 2)
					{
						$this->query("UPDATE " . NPDB_PREFIX . "users SET balance=balance-" . $invoice_details['amount'] . " WHERE 
	...
	function relist_reverse($reverse_id, $user_id, $duration = 0, $charge_fees = true, $auto_relist = false)
	{
		...
		$item_details = $this->get_sql_row("SELECT r.*, u.user_id FROM " . DB_PREFIX . "reverse_auctions r
			LEFT JOIN " . NPDB_PREFIX . "users u ON r.owner_id=u.user_id 
	...
	function reverse_winners_show($item_details)
	{
		...
		$sql_select_winners = $this->query("SELECT w.bid_amount, u.username, u.user_id FROM " . DB_PREFIX . "reverse_winners w
				LEFT JOIN " . NPDB_PREFIX . "users u ON w.provider_id=u.user_id WHERE
	...
	function reverse_pmb_link($item_details, $user_id)
	{
		...
		if ($is_winner)
		{
			$sql_select_wins = $this->query("SELECT w.*, b.username AS buyer_username, s.username AS seller_username, rb.bid_id FROM " . DB_PREFIX . "reverse_winners w
				...
				LEFT JOIN " . NPDB_PREFIX . "users b ON w.provider_id=b.user_id
				LEFT JOIN " . NPDB_PREFIX . "users s ON w.poster_id=s.user_id	WHERE
