...
class fees extends tax
{
 ...
 //$post_url = 'https://www.paypal.com/cgi-bin/webscr';
        ...
        file_put_contents('pp_paypal_in.txt', "url:".$post_url."  email:".$paypal_email);
...
#what percent do we give back. check in the config_giveback file
include('config_giveback.php');

$points=($payment_amount*$givebackpercent);
		$display_output .= '<h2 style="font-family:verdana;font-size:120%;color:red">This payment will earn <strong>' .$this->display_amount($points) . '</strong>  Bring It Local Giveback Points</h2></td></tr></table><br>';
 ...
 #               $paypal_email = 'support@bringitlocal.com';
	...
	function callback_process($custom_id, $fee_table, $payment_gateway, $payment_amount, $txn_id = null, $currency = null)
	{
		...
		{
            $activity_sql="SELECT points_awarded FROM probid_user_activities WHERE activity_id = 7";
            ...
            mysql_query("INSERT INTO probid_user_points (user_id, activity_id, points_awarded, awarded_date)
                              VALUES ('$custom_id', '7', '$points_awarded', date(now()))");

            $email_user_id = $custom_id;
            $activity_id = 7;
...
##mw-3-6-2011
##calculate giveback points and allocate to the right NP. If its an end of auction fee divide the points between buyer and seller nps
##first- we need to search the user table to find the matching np for the seller or if end of auction for both buyer np and seller np
##we'll get the adjustable items, like the percent we giveback from this config file
include('config_giveback.php');
$sellernp_id = $this->get_sql_field("SELECT npuser_id FROM " . DB_PREFIX . "users WHERE user_id='" . $user_id . "'", npuser_id);
$np_name = $this->get_sql_field("SELECT npname FROM " . DB_PREFIX . "users WHERE user_id='" . $user_id . "'", npname);
$username = $this->get_sql_field("SELECT username FROM " . DB_PREFIX . "users WHERE user_id='" . $user_id . "'", username);

##total giveback points for the transaction
$points=($payment_amount * $givebackpercent);

##it all goes to the sellernp for auction set up fees-for end of auction fees we divide the points
$sellernp_points=$points;

## then we insert them to the invoice db
$today = date("Ymd");  	

				$sql_insert_invoice = $this->query("INSERT INTO " . DB_PREFIX . "invoices
					(user_id, item_id, name, amount, invoice_date, current_balance, live_fee, processor, tax_amount, tax_rate, tax_calculated, sellernp_id, buyernp_id, points, sellernp_points, buyernp_points) VALUES
					('" . $user_id . "', '" . $custom_id . "', '" . $invoice_name . "',
					'" . $payment_amount . "', '" . CURRENT_TIME . "', '0', '1', '" . $payment_gateway . "', '" . $tax_settings['amount'] . "', '" . $tax_settings['tax_rate'] . "', '1' , '" . $sellernp_id . "', '" . $buyernp_id . "', '" . $points . "', '" . $sellernp_points . "', '" . $buyernp_points . "')");



#and now into the giveback_invoices table
$invoice_id = $this->insert_id();
$sql_insert_invoice = $this->query("INSERT INTO giveback_invoices
					(np_userid, np_name, points, invoice_date, user_id, username, invoice_id, buyerorseller, transtype) VALUES
					('" . $sellernp_id . "', '" . $np_name . "', '" . $points . "', '" . $today  . "', '" . $user_id  . "', '" . $username  . "', '" . $invoice_id . "', '1' , '" . $invoice_name . "')");
...
##we'll get the adjustable items, like the percent we giveback from this config file
include('config_giveback.php');
##find who is the buyer and seller
$user_id = $winner_details['seller_id'];
$buyer_id = $winner_details['buyer_id'];
$sellerusername = $this->get_sql_field("SELECT username FROM " . DB_PREFIX . "users WHERE user_id='" . $user_id . "'", username);
$buyerusername = $this->get_sql_field("SELECT username FROM " . DB_PREFIX . "users WHERE user_id='" . $buyer_id . "'", username);

##then- we need to search the user table to find the matching np for the seller or if end of auction for both buyer np and seller np
$sellernp_id = $this->get_sql_field("SELECT npuser_id FROM " . DB_PREFIX . "users WHERE user_id='" . $user_id . "'", npuser_id);
$buyernp_id = $this->get_sql_field("SELECT npuser_id FROM " . DB_PREFIX . "users WHERE user_id='" . $buyer_id . "'", npuser_id);
$sellernp_name = $this->get_sql_field("SELECT npname FROM " . DB_PREFIX . "users WHERE user_id='" . $user_id . "'", npname);
$buyernp_name = $this->get_sql_field("SELECT npname FROM " . DB_PREFIX . "users WHERE user_id='" . $buyer_id . "'", npname);

##total giveback points for the transaction
$points=($payment_amount * $givebackpercent);

##sellernp and buyernp giveback points are allocated to both for the end of auction fees transaction
$sellernp_points=($points * $sellernp_percent);
$buyernp_points=($points * $buyernp_percent);

## then we add them all to the insert sql
		
	$sql_insert_invoice = $this->query("INSERT INTO " . DB_PREFIX . "invoices (user_id, item_id, name, amount, invoice_date, current_balance, 
live_fee, processor, tax_amount, tax_rate, tax_calculated, sellernp_id, buyernp_id, points, sellernp_points, buyernp_points) VALUES 
('" . $payer_id . "', '" . $winner_details['auction_id'] . "', '" . $invoice_name . "', '" . $payment_amount . "', '" . 
CURRENT_TIME . "', '0', '1', '" . $payment_gateway . "', '" . $tax_settings['amount']. "', '" . $tax_settings['tax_rate'] . "', '1' , '" . 
$sellernp_id . "', '" . $buyernp_id . "', '" . $points . "', '" . $sellernp_points . "', '" . $buyernp_points . "')");


				$invoice_id = $this->insert_id();

## now create a giveback_invoice for both buyer and seller

## seller giveback invoice
$today = date("Ymd");  	
$sql_insert_invoice = $this->query("INSERT INTO giveback_invoices
					(np_userid, np_name, points, invoice_date, user_id, username, invoice_id, buyerorseller, transtype) VALUES
					('" . $sellernp_id . "', '" . $sellernp_name . "', '" . $sellernp_points . "', '" . $today  . "', '" . $user_id  . "', '" . $sellerusername  . "', '" . $invoice_id . "', '1' , '" . $invoice_name . "')");

## buyer giveback invoice
$sql_insert_invoice = $this->query("INSERT INTO giveback_invoices
					(np_userid, np_name, points, invoice_date, user_id, username, invoice_id, buyerorseller, transtype) VALUES
					('" . $buyernp_id . "', '" . $buyernp_name . "', '" . $buyernp_points . "', '" . $today  . "', '" . $buyer_id  . "', '" . $buyerusername  . "', '" . $invoice_id . "', '0' , '" . $invoice_name . "')");
...
##calculate giveback points and allocate to the right NP. 
##first- we need to search the user table to find the matching np for the seller or if end of auction for both buyer np and seller np
##we'll get the adjustable items, like the percent we giveback from this config file
include('config_giveback.php');
$sellernp_id = $this->get_sql_field("SELECT npuser_id FROM " . DB_PREFIX . "users WHERE user_id='" . $user_id . "'", npuser_id);
...
##it all goes to the sellernp for want ad set up fees-for end of auction fees we divide the points
$sellernp_points=$points;
...
$today = date("Ymd");  	
				...
				$sql_insert_invoice = $this->query("INSERT INTO " . DB_PREFIX . "invoices
					(user_id, wanted_ad_id, name, amount, invoice_date, current_balance, live_fee, processor, tax_amount, tax_rate, tax_calculated,sellernp_id, buyernp_id, points, sellernp_points, buyernp_points) VALUES
					('" . $user_id . "', '" . $custom_id . "', '" . $invoice_name . "',
					'" . $payment_amount . "', '" . CURRENT_TIME . "', '0', '1', '" . $payment_gateway . "', '" . $tax_settings['amount'] . "', '" . $tax_settings['tax_rate'] . "', '1' , '" . $sellernp_id . "', '" . $buyernp_id . "', '" . $points . "', '" . $sellernp_points . "', '" . $buyernp_points . "')");
...
##we'll get the adjustable items, like the percent we giveback from this config file
include('config_giveback.php');
#get the sellers npuser_id and	npname
$sellernp_id = $this->get_sql_field("SELECT npuser_id FROM " . DB_PREFIX . "users WHERE user_id='" . $custom_id . "'", npuser_id);
$np_name = $this->get_sql_field("SELECT npname FROM " . DB_PREFIX . "users WHERE user_id='" . $custom_id . "'", npname);
$sellerusername = $this->get_sql_field("SELECT username FROM " . DB_PREFIX . "users WHERE user_id='" . $custom_id . "'", username);
##total giveback points for the transaction
$points=($payment_amount * $givebackpercent);


############
			$sql_insert_invoice = $this->query("INSERT INTO " . DB_PREFIX . "invoices (user_id, name, amount, invoice_date, current_balance, live_fee, processor, tax_amount, tax_rate, tax_calculated, sellernp_id, buyernp_id, points, sellernp_points, buyernp_points) VALUES('" . $custom_id . "', '" . $invoice_name . "', '" . $payment_amount . "', '" . CURRENT_TIME . "', '0', '1', '" . $payment_gateway . "', '" . $tax_settings['amount']. "', '" . $tax_settings['tax_rate'] . "', '1',  '" . $sellernp_id . "', '" . $buyernp_id . "', '" .  $points . "', '" . $sellernp_points . "', '" .  $buyernp_points . "')");
...
$sql_insert_invoice = $this->query("INSERT INTO giveback_invoices
					(np_userid, np_name, points, invoice_date, user_id, username, invoice_id, buyerorseller, transtype) VALUES
					('" . $sellernp_id . "', '" . $np_name . "', '" . $points . "', '" . $today  . "', '" . $custom_id  . "', '" . $sellerusername  . "', '" . $invoice_id . "', '1' , '" . $invoice_name . "')");

$this->query("UPDATE " . DB_PREFIX . "users SET seller_verified_pending=1, seller_verif_last_payment='" . CURRENT_TIME . "', 
				seller_verif_next_payment='" . $seller_verif_next_payment . "' WHERE user_id=" . $custom_id);
			...
			$site_details = $this->get_sql_row("SELECT sitename, admin_email FROM " . DB_PREFIX . "gen_setts");
				 
		
			$send = true; // always sent;
			## text message - editable
			$text_message = 'Dear %1$s, <br>
							We have received your request to be verified as a seller. 
							<br>We will be contacting you shortly to complete your seller verification.
							<br>
							Best regards,<br>
							The %3$s staff';
			...
			## mail content to administrator
			$text_message1 = sprintf($text_message, 'Administrator', $sellerusername, $site_details['sitename']);

			$html_message1 = sprintf($text_message, 'Administrator', $sellerusername, $site_details['sitename']);

			## mail content to requester
			$text_message = sprintf($text_message, 'Administrator', $sellerusername, $site_details['sitename']);

			$html_message = sprintf($html_message, $sellerusername,$sellerusername, $site_details['sitename']);

			send_mail($site_details['admin_email'], $site_details['sitename'] . ' - seller verification request for ' . $sellerusername, $text_message1, $site_details['admin_email'], $html_message1, null, $send);

			send_mail($user_details['email'], $site_details['sitename'] . ' - seller verification request for ' . $sellerusername, $text_message,  $site_details['admin_email'], $html_message, null, $send);
		...
		else if ($fee_table == 7) ## cart sale fee payment - live fee only
		{
			$sql_update_winner = $this->query("UPDATE " . DB_PREFIX . "winners SET
				active=1, payment_status='confirmed', live_pm_amount='" . $payment_amount . "',
				live_pm_date='" . CURRENT_TIME . "', live_pm_processor='" . $payment_gateway . "' WHERE sc_id='" . $custom_id . "'");

			## now add a live payment item setup invoice (live_fee = 1)
			$sc_details = $this->get_sql_row("SELECT sc.* FROM " . DB_PREFIX . "shopping_carts sc 
				WHERE sc.sc_id='" . $custom_id . "'");

			if ($sc_details['sc_id'] > 0)
			{
				$this->set_fees($sc_details['seller_id']);

				$payer_id = (eregi('b', $this->fee['endauction_fee_applies'])) ? $sc_details['buyer_id'] : $sc_details['seller_id'];

				$invoice_name = GMSG_SC_SALE_FEE . ' - ' . MSG_SC_ID . ': ' . $sc_details['sc_id'];
...
##mw-3-6-2011
##calculate giveback points and allocate to the right NP. this is a store subscription so all points go to the buyer like an auction setup
##first- we need to search the user table to find the matching np for the seller or if end of auction for both buyer np and seller np
##we'll get the adjustable items, like the percent we giveback from this config file
include('config_giveback.php');
$sellernp_id = $this->get_sql_field("SELECT npuser_id FROM " . DB_PREFIX . "users WHERE user_id='" . $custom_id . "'", npuser_id);
$np_name = $this->get_sql_field("SELECT npname FROM " . DB_PREFIX . "users WHERE user_id='" . $custom_id . "'", npname);
$username = $this->get_sql_field("SELECT username FROM " . DB_PREFIX . "users WHERE user_id='" . $custom_id . "'", username);
##total giveback points for the transaction
$points=($payment_amount * $givebackpercent);
##it all goes to the sellernp for store subscription fees
$sellernp_points=$points;
## then we insert them to the invoice db

			
			$sql_insert_invoice = $this->query("INSERT INTO " . DB_PREFIX . "invoices
				(user_id, name, amount, invoice_date, current_balance, live_fee, processor, tax_amount, tax_rate, tax_calculated, sellernp_id, buyernp_id, points, sellernp_points, buyernp_points) VALUES
...
$sql_insert_invoice = $this->query("INSERT INTO giveback_invoices
					(np_userid, np_name, points, invoice_date, user_id, username, invoice_id, buyerorseller, transtype) VALUES
					('" . $sellernp_id . "', '" . $np_name . "', '" . $points . "', '" . $today  . "', '" . $custom_id  . "', '" . $username  . "', '" . $invoice_id . "', '1' , '" . $invoice_name . "')");
		...
		else if ($fee_table == 110) ## direct payment - shopping cart
		{
			$sql_update_sc = $this->query("UPDATE " . DB_PREFIX . "shopping_carts SET
				flag_paid=1, direct_payment_paid=1 WHERE sc_id='" . $custom_id . "'");						

			$sql_update_winner = $this->query("UPDATE " . DB_PREFIX . "winners SET
				flag_paid=1, direct_payment_paid=1, dd_active=IF(is_dd=1, 1, 0), 
				dd_active_date=IF(is_dd=1, " . CURRENT_TIME . ", 0) WHERE sc_id='" . intval($custom_id) . "'");			
		}

        //add points for sell and by items
        if($fee_table == 50 OR $fee_table == 100){
            $winner_details = $this->get_sql_row("SELECT w.*, a.category_id FROM " . DB_PREFIX . "winners w
				LEFT JOIN " . DB_PREFIX . "auctions a ON a.auction_id=w.auction_id WHERE w.winner_id='" . $custom_id . "'");
            $seller_id = $winner_details['seller_id'];
            $buyer_id = $winner_details['buyer_id'];

            //add points for seller
            $sell_item_sql="SELECT points_awarded FROM probid_user_activities WHERE activity_id = 2";
            $sell_item_result = mysql_query($sell_item_sql);
            $sell_item_row = mysql_fetch_array($sell_item_result);
            $sell_item_points_awarded = $sell_item_row['points_awarded'];
            mysql_query("INSERT INTO probid_user_points (user_id, activity_id, points_awarded, awarded_date)
                                                          VALUES ('$seller_id', '2', '$sell_item_points_awarded', date(now()))");

            $email_user_id = $seller_id;
            $activity_id = 2;
            include('global_loyality_user_notification.php');

            //add points for buyer
            $buy_item_sql="SELECT points_awarded FROM probid_user_activities WHERE activity_id = 5";
            $buy_item_result = mysql_query($buy_item_sql);
            $buy_item_row = mysql_fetch_array($buy_item_result);
            $buy_item_points_awarded = $buy_item_row['points_awarded'];
            mysql_query("INSERT INTO probid_user_points (user_id, activity_id, points_awarded, awarded_date)
                                                          VALUES ('$buyer_id', '5', '$buy_item_points_awarded', date(now()))");

            $email_user_id = $buyer_id;
            $activity_id = 5;
            include('../global_loyality_user_notification.php');
        }
...
$sellernp_id = $this->get_sql_field("SELECT npuser_id FROM " . DB_PREFIX . "users WHERE user_id='" . $user_id . "'", npuser_id);
##total giveback points for the transaction
$points=($output['amount'] * $givebackpercent);
##it all goes to the sellernp for store subscription fees
$sellernp_points=$points;
## then we insert them to the invoice db
				...
				$sql_insert_invoice = $this->query("INSERT INTO " . DB_PREFIX . "invoices
					(user_id, name, amount, invoice_date, current_balance, live_fee, sellernp_id, buyernp_id, points, sellernp_points, buyernp_points) VALUES
					('" . $user_details['user_id'] . "', '" . $invoice_name . "', '" . $output['amount'] . "',
					'" . CURRENT_TIME . "', '" . $account_balance . "', '1' , '" . $sellernp_id . "', '" . $buyernp_id . "', '" . $points . "', '" . $sellernp_points . "', '" . $buyernp_points . "')");
	...
	function cart_sale_payment($sc_id, $user_id)
	{
		(array) $output = null; /* we dont need an array here since we always display this functions output */
		(int) $fee_table = 7; // clear account balance - table = 2 (DB_PREFIX->users)

		$this->set_fees($payer_id); 

		$this->user_id = $payer_id; ## required for the protx gateway
		$user_details = $this->get_sql_row("SELECT * FROM " . DB_PREFIX . "users WHERE user_id=" . $user_id);

		$sql_select_cart = $this->query("SELECT sci.quantity, a.buyout_price AS price 
			FROM " . DB_PREFIX . "shopping_carts_items sci 
			LEFT JOIN " . DB_PREFIX . "auctions a ON a.auction_id=sci.item_id 
			WHERE sci.sc_id=" . $sc_id);
		
		$cart_total = 0;
		while ($sc_details = $this->fetch_array($sql_select_cart)) 
		{
			$cart_total += $sc_details['price'] * $sc_details['quantity'];
		}
				
		$sale_fee = $this->display_fee_tiers('endauction', $user_details, 0, 'both', $cart_total);
		
		$output['amount'] = $sale_fee[0]['amount'];
		$purchase_fee_desc = GMSG_CART_SALE_FEE_PAYMENT . ' [ ' . MSG_SC_ID . ': ' . $sc_id . ' ]';

		if ($output['amount'] > 0)
		{
				$output['display'] = $this->payment_message($output['amount'], 0, MSG_TO_PAY_FOR_SC_SALE_FEE);

				$transaction_id = $sc_id . 'TBL' . $fee_table;

				$payment_description = $this->setts['sitename'] . ' - ' . $purchase_fee_desc;

				$output['display'] .= $this->show_gateways($transaction_id, $output['amount'], $this->setts['currency'], 0, $payment_description);
		}
		else
		{
			## all we need to actually do is activate the shopping cart row
			$sql_update_winner = $this->query("UPDATE " . DB_PREFIX . "winners SET
				active=1, payment_status='confirmed' WHERE sc_id='" . $sc_id . "'");

			$output['display'] = '<table class="errormessage" align="center"><tr><td align="center"> '.
				'<p class=contentfont align=center>' . MSG_THE_SALE . ' #' . $sc_id . ' ' . MSG_HAS_BEEN_ACTIVATED . '</p>'.
				'</td></tr></table>';
		}
