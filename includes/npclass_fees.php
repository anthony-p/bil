...
class npfees extends nptax
{
	...
	function form_paypal ($transaction_id, $paypal_email, $payment_amount, $currency, $payment_description=null, $direct_payment=false, $post_url='https://www.paypal.com/cgi-bin/webscr')
	{
		...
		{
			$user_details = $this->get_sql_row("SELECT * FROM " . NPDB_PREFIX . "users WHERE 
	...
	function form_protx ($transaction_id, $protx_username, $protx_password, $payment_amount, $currency, $direct_payment=false, $post_url='https://ukvps.protx.com/vps2form/submit.asp')
	{
		...
		$user_details = $this->get_sql_row("SELECT email, address, zip_code FROM " . NPDB_PREFIX . "users WHERE 
	...
	function set_fees($user_id, $category_id = 0)
	{
		...
		$preferred_seller = $this->get_sql_field("SELECT preferred_seller FROM " . NPDB_PREFIX . "users WHERE
	...
	function show_gateways($transaction_id, $payment_amount, $currency, $user_id, $payment_description = null, $dp_gateways = null)
	{
		...
		{
			$direct_payment = true;
			$pg_details = $this->get_sql_row("SELECT * FROM " . NPDB_PREFIX . "users WHERE user_id=" . $user_id);
	...
	function callback_process($custom_id, $fee_table, $payment_gateway, $payment_amount, $txn_id = null, $currency = null)
	{
		...
		if ($fee_table == 1) ## signup process - alter 'users' table
		{
			...
			$this->query("UPDATE " . NPDB_PREFIX . "users SET active=1, approved=1, payment_status='confirmed', mail_activated=1 WHERE user_id=" . $custom_id);
		...
		else if ($fee_table == 2) ## clear account balance - alter 'users' table
		{
			$user_details = $this->get_sql_row("SELECT balance, active FROM
				" . NPDB_PREFIX . "users WHERE user_id=" . intval($custom_id));
			...
			$this->query("UPDATE " . NPDB_PREFIX . "users SET active=1, balance=" . $balance . " WHERE user_id=" . intval($custom_id));
		...
		{
			$user_details = $this->get_sql_row("SELECT f.*, u.seller_verif_next_payment FROM " . DB_PREFIX . "fees f, " . NPDB_PREFIX . "users u WHERE 
			...
			$this->query("UPDATE " . NPDB_PREFIX . "users SET seller_verified=1, seller_verif_last_payment='" . CURRENT_TIME . "', 
		...
		{
			$shop_details = $this->get_sql_row("SELECT f.*, u.shop_next_payment FROM " . DB_PREFIX . "fees_tiers f, " . NPDB_PREFIX . "users u WHERE 
			...
			$this->query("UPDATE " . NPDB_PREFIX . "users SET shop_active=1, shop_last_payment='" . CURRENT_TIME . "', 
		...
		else if ($fee_table == 50) ## direct payment - single item
		{
			...
			if (!$winner_details['invoice_sent'] || $is_invoice > 1)
			{
				...
				$this->query("UPDATE " . DB_PREFIX . "winners SET 
					invoice_sent='1', invoice_id='" . $invoice_id . "', 
   		...
   		foreach ($setup_fee as $key => $value)
   		{
   			if ($value['amount'])
   			{
   				...
   				if ($user_payment_mode == 2 && $add_invoices)
   				{
   					...
   					$sql_update_user_balance = $this->query("UPDATE " . NPDB_PREFIX . "users SET
	...
	function prepare_rollback($auction_id, $user_id)
	{
		...
		$user_balance = $this->get_sql_field("SELECT balance FROM " . NPDB_PREFIX . "users WHERE user_id='" . $user_id . "'", 'balance');
	...
	function sale($winning_bid_details, $item_details)
	{
		...
		$user_details = $this->get_sql_row("SELECT * FROM " . NPDB_PREFIX . "users WHERE user_id=" . $payer_id);
	...
	function store_subscription($shop_account_id, $user_id)
	{
		...
		{		
			...
			$this->query("UPDATE " . NPDB_PREFIX . "users SET shop_active=1, shop_last_payment='" . $shop_last_payment . "', 
				shop_next_payment='" . $shop_next_payment . "' WHERE user_id=" . $user_id);

			## if there is no fee we will activate the store
			$sql_update_store = $this->query("UPDATE " . NPDB_PREFIX . "users SET
