...
class messaging extends database
{
	 ...
	 */
	function new_topic($auction_id, $sender_id, $receiver_id, $is_question, $message_title, $message_content, $message_handle, $winner_id=0, $admin_message = 0, $sc_id=0)
	{
		...
		{
			$this->query("UPDATE " . DB_PREFIX . "shopping_carts SET messaging_topic_id=" . $topic_id . " WHERE sc_id=" . $sc_id);			
		}
		...
		if ($topic_details)
		{
			...
			else ## above condition not met, INSERT message
			{
				$sql_insert_reply = $this->query("INSERT INTO " . DB_PREFIX . "messaging
					...
					message_handle, winner_id, wanted_ad_id, reverse_id, bid_id, sc_id) VALUES
					...
					'" . $topic_details['wanted_ad_id'] . "', '" . $topic_details['reverse_id'] . "', '" . $topic_details['bid_id'] . "', '" . $topic_details['sc_id'] . "')");
	...
	function delete_message($message_id, $user_id, $deletion_type)
	{
		...
		if ($deletion_type == 'receiver_deleted')
		{
			...
			$sql_query = "UPDATE " . DB_PREFIX . "messaging SET receiver_deleted=1 WHERE
				message_id='" . intval($message_id) . "' AND receiver_id='" . intval($user_id) . "'";
		}
		else if ($deletion_type == 'sender_deleted')
		{
			$run_query = true;
			$sql_query = "UPDATE " . DB_PREFIX . "messaging SET sender_deleted=1 WHERE
				message_id='" . intval($message_id) . "' AND sender_id='" . intval($user_id) . "'";
		}
	...
	function message_subject($message_details) ## it is presumed the auction name is available in the array, message_handle as well
	{
		...
		switch ($message_details['message_handle'])
		{
			...
			case 15:
				...
				break;
			case 6:
				$display_output = MSG_SHOPPING_CART_SALE . '<br>' .
					MSG_SC_ID . ': ' . $message_details['sc_id'];
				break;
			default:
	...
	function msg_board_link($message_details)
	{
		...
		switch ($message_details['message_handle'])
		{
			...
			case 15:
				...
				break;
			case 6:
				$output = process_link('message_board', array('topic_id' => $message_details['topic_id'], 'sc_id' => $message_details['sc_id']));
				break;
			default:
	...
	{
		$id_field = (in_array($message_handle, array(4, 5))) ? 'wanted_ad_id' : 'auction_id';
		$id_field = ($message_handle == 6) ? 'sc_id' : $id_field;
		$id_field = (in_array($message_handle, array(15))) ? 'reverse_id' : $id_field;
