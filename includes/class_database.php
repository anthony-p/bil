...
#added mw to be allow the landing page to search the np user table
	function count_nprows($table_name, $condition = null, $debug = false)
...
#added mw to be allow the landing page to search the giveback_invoices table
	function count_givebackrows($table_name, $condition = null, $debug = false)
          
	{
		$query_result = $this->query("SELECT count(*) AS count_rows FROM " . $this->givebackDB_PREFIX . $table_name . " " . $condition, $debug);
		$count_rows = $this->sql_result($query_result, 0, 'count_rows');
