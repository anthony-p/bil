...
class projectRewards extends custom_field {
	...
	//--------------------------------------------------------------------------------------------------------------------------
	function getAllRewards($campaign_id){
		$project_rewards = array();
		if(!empty($campaign_id)){
			$sql = "SELECT * FROM project_rewards r LEFT JOIN np_users np ON r.project_id = np.user_id LEFT JOIN bl2_users u ON np.probid_user_id =  u.id WHERE r.project_id=" . $campaign_id . " ORDER BY r.id DESC";
