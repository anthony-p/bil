...
{
function insert_global_partner($variables_array)
	{
	
	ads_counter($cnt_details['category_id'], 'add', $cnt_details['advert_id']);
				ads_counter($cnt_details['addl_category_id'], 'add', $cnt_details['advert_id']);
				
		$post_details = $this->rem_special_chars_array($variables_array);

		$result = $this->query("INSERT INTO " . DB_PREFIX . "partners
			(advert_url, advert_img_path, advert_alt_text, advert_text_under, views_purchased, clicks_purchased,
			category_id, advert_code, advert_type, section_id, hpfeat,catfeat,approved, active,name, description, advert_pct) VALUES
			...
			'" . $this->implode_array($variables_array['categories_id']) . "', '" . 
			$post_details['advert_code'] . "', '" . 
			$post_details['advert_type'] . "', '" .
			$post_details['section_id'] . "', '" .
			$post_details['hpfeat'] . "', '" .
			$post_details['catfeat']. "', '" .
			$post_details['approved']. "', '" .
			$post_details['active'] . "', '" .
			$post_details['name'] . "', '" .
			$post_details['description'] . "', '" .
			$post_details['advert_pct'] . "')");
	...
	function edit_global_partner($variables_array, $advert_id)
	{
		$post_details = $this->rem_special_chars_array($variables_array);

		$result = $this->query("UPDATE " . DB_PREFIX . "partners SET
			...
			category_id='" . $this->implode_array($variables_array['categories_id']) . "', advert_code='" . $post_details['advert_code'] . "',	advert_type='" . $post_details['advert_type']. "', 	advert_pct='" . $post_details['advert_pct']. "', 	
			section_id='" .  $post_details['section_id'] . "' ,hpfeat='" .  $post_details['hpfeat'] . "' ,catfeat='" .  $post_details['catfeat'] . "' ,active='" .  	$post_details['active']
			. "' ,approved='" .  	$post_details['approved']	. "' ,name='" .  	$post_details['name']	. "' ,description='" .  	$post_details['description'] ."' ,big_banner_code='" .$post_details['big_banner_code']
						. "' 	WHERE advert_id='" . $advert_id . "'");
																								
		return result;
	}

	function delete_global_partner($advert_id)
	{
		$result = $this->query("DELETE FROM " . DB_PREFIX . "partners WHERE advert_id='" . $advert_id . "'");

		return $result;
	}

	function global_partner_details($advert_row)
	{
	...
	function display_global_partner($advert_row, $in_admin = false)
	{
		(string) $display_output = null;
		...
		if (!$in_admin)
		{
			$add_view = $this->query("UPDATE " . DB_PREFIX . "partners SET views=views+1 WHERE advert_id='" . $advert_row['advert_id'] . "'");
		}
	...
	function select_global_partner($base_url, $parent_id, $auction_id, $section_id = 0)
	{
		(string) $display_output = null;
	...
	function display_banner($advert_row, $in_admin = false)
	{
		...
		{
			$display_output = $this->add_special_chars($advert_row['advert_code']).$this->add_special_chars($advert_row['big_banner_code']);
		}
