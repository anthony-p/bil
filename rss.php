...
define('RSSlogo', 'images/bringitlocalogo.gif'); // logo must be relative to site url
...
switch ($feed)
{
	...
	case 10:
		$RSStitle  = $db->get_sql_field("SELECT username FROM np_users WHERE user_id='" . $user_id . "'", 'username');
		$postdate = 'start_time';
		$sort = 'DESC';
		$subquery = 'start_time<=' . CURRENT_TIME . ' AND buyout_price>0';
		break;
...
if ($feed==10)
{
		...
		WHERE  a.closed=0 AND a.active=1 AND a.approved=1 AND a.deleted=0 and a.npuser_id=" .  $user_id . " union " .
		" SELECT a.wanted_ad_id as auction_id, a.name, a.start_time AS postdate, a.description, 0 as max_bid, 0 as nb_bids, '$' as currency, 'wanted_ads' as category_id, 0 as closed, 0 as bold, 0 as hl, 0 as buyout_price, 0 as is_offer, 0 as start_price, 
		0 as reserve_price, '' as media_url, '' as username FROM probid_wanted_ads as a where npuser_id=" . $user_id .
		" union " .
		" SELECT a.news_id as auction_id, a.news_name as name, a.reg_date AS postdate, a.news_content as description, 0  as max_bid, 
		0 as nb_bids, '$' as currency, 'news' as category_id, 0 closed, 0 as bold, 0 as hl, 0 as buyout_price, 0 as is_offer, 0 as start_price, 
		0 as reserve_price, '' as media_url, '' as username FROM np_news a where user_id=" . $user_id);
}
else
{
	$results = $db->query("SELECT a.auction_id, a.name, a." . $postdate . " AS postdate, a.description, a.max_bid, 
...
while ($row=@mysql_fetch_assoc($results)) // loop through database fields & build item array
{
	...
	{
			case 'wanted_ads':
				$items['link'][]		= utf8_encode(SITE_PATH .'wanted_details.php?wanted_ad_id=' . $row['auction_id']);
				break;
			case 'news':
				$items['link'][]		= utf8_encode(SITE_PATH . 'npnews.php?news=news&news_id=' . $row['auction_id']);
				break;
			default:
...
<channel>
	<title>{$setts['sitename']} - $RSStitle</title>
	<link>{$setts['site_path']}</link>
	<description>{$setts['sitename']}</description>
    <lastBuildDate>Mon, 13 Jun 2011 12:59:27 GMT</lastBuildDate>  
<image>
...
<item>
	...
	<link><![CDATA[ {$items['link'][$i]} ]]></link>
	<guid isPermaLink="true"><![CDATA[ {$items['link'][$i]} ]]> </guid>
