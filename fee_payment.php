...
else
{
	...
	else if ($_REQUEST['do'] == 'sc_sale_fee_payment')
	{
		$sc_id = intval($_REQUEST['sc_id']);
		$fee_type_message = ' - ' . GMSG_CART_SALE_FEE_PAYMENT . ' [ ' . MSG_SC_ID . ': ' . $sc_id . ' ]';

		$sc_sale_fee = new fees();
		$sc_sale_fee->setts = &$setts;
		$sc_sale_fee->set_fees($session->value('user_id'));

		$payer_field = (eregi('b', $sc_sale_fee->fee['sa_sale_fee_applies'])) ? 'buyer_id' : 'seller_id';
		
		$sc_details = $db->get_sql_row("SELECT * FROM " . DB_PREFIX . "shopping_carts WHERE sc_id='" . $sc_id . "' AND 
			" . $payer_field . "=" . $session->value('user_id') . " AND sc_purchased=1");
		
		if ($sc_details['sc_id'])
		{
			$output = $sc_sale_fee->cart_sale_payment($sc_id, $session->value('user_id'));
			$payment_table_display = $output['display'];
		}
		else 
		{
			$payment_table_display = MSG_ERROR_SC_SALE_PAYMENT;			
		}
		$template->set('payment_table_display', $payment_table_display);
	...
	else if ($_REQUEST['do'] == 'sc_direct_payment')
	{
		$fee_type_message = ' - ' . MSG_SHOPPING_CART_DIRECT_PAYMENT;
		...
		$fee_output = $cart->sc_direct_payment_box($sc_id);

		$template->set('payment_table_display', $fee_output);
