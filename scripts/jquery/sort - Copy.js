...
//now, for each select box, run this function
    $("select.changeMe").each(function(){
        ...
        //write the default
        var gddFirst = '<div class="first"><span class="selectME gselected" id="'+ curSel.attr('id') +'" style="width:'+ gddWidth2 +  'px;">'+ whatSelected +'</span><span id="arrowImg"></span><div class="clears"></div></div><ul class="selectME">';
        // create a new array of div options from the original's options
        var addItems = new Array();
        $(curSel).children('option').each( function() {
            var text = $(this).text();
            var selVal = $(this).attr('value');
            var gddClass = $(this).attr('class');
            var before =  '<li style="width:' + gddWidthL + 'px;"><a href="#" class="' + gddClass + '" rel="' + selVal + '" tabindex="0"  style="width:' + gddWidth3 + 'px;" onclick="select('+ "'" + gddClass + "'" +',this)">';
            var after = '</a></li>';
