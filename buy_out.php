...
$GLOBALS['body_id'] = "buy_out";


$item_id = intval($_REQUEST['auction_id']);
if (is_shopping_cart($item_id)) /* if the item will be added to a shopping cart, redirect */
{
	header_redirect('shopping_cart.php?item_id=' . $item_id);
}
...
{
    header_redirect('extended_registration.php?redirect=auction_details.php?auction_id=' . $_REQUEST['auction_id']);
}
else
{
	...
	{## PHP Pro Bid v6.00 check if another bid is in progress, if not mark as in progress
		...
		$purchase_result = $item->assign_winner($item_details, 'buy_out', $session->value('user_id'), $quantity, null, $force_payment_enabled);
