...
if ($setts['enable_stores'])
{
	...
	$header_stores_page = header5(MSG_LOCALSTORES_DIRECTORY);
...
$np_userid = $_COOKIE['np_userid'];		
	$template->set('search_options_title', MSG_LOCALSTORE_SEARCH);
	$store_search_box = $template->process('localsearch.tpl.php');
	...
	$pagination = paginate($start, $limit, $nb_stores, 'local_stores.php', $additional_vars . $order_link);
	...
	$sql_select_stores = $db->query("SELECT user_id, shop_name, shop_logo_path, shop_mainpage, username, shop_nb_items FROM " . DB_PREFIX . "users WHERE
		active=1 AND shop_active=1 AND npuser_id='$np_userid' " . $addl_store_query . " 
