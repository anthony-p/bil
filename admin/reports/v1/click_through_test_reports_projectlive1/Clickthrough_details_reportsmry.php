...
$Clickthrough_details_report = NULL;

//
// Table class for Clickthrough details report
//
...
$Clickthrough_details_report_summary = NULL; // Initialize page object first
...
// Create page object
var Clickthrough_details_report_summary = new ewr_Page("Clickthrough_details_report_summary");

// page properties
Clickthrough_details_report_summary.PageID = "summary"; // page ID
var EWR_PAGE_ID = Clickthrough_details_report_summary.PageID;

// extend page with Chart_Rendering function
Clickthrough_details_report_summary.Chart_Rendering =  
...
Clickthrough_details_report_summary.Chart_Rendered =  
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var fClickthrough_details_reportsummary = new ewr_Form("fClickthrough_details_reportsummary");

// Validate method
fClickthrough_details_reportsummary.Validate = function(fobj) {
...
fClickthrough_details_reportsummary.Form_CustomValidate =  
...
fClickthrough_details_reportsummary.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
fClickthrough_details_reportsummary.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// Use Ajax
fClickthrough_details_reportsummary.Lists["sv_user_name"] = {"LinkField":"sv_user_name","DisplayFields":["sv_user_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fClickthrough_details_reportsummary.Lists["sv_np2Dname"] = {"LinkField":"sv_np2Dname","DisplayFields":["sv_np2Dname","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
</script>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fClickthrough_details_reportsummary" id="fClickthrough_details_reportsummary" class="ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>" onsubmit="return ewrForms[this.id].Submit();">
<input type="hidden" name="cmd" value="search">
<a href="javascript:fClickthrough_details_reportsummary.ToggleSearchPanel();" style="text-decoration: none;"><img id="fClickthrough_details_reportsummary_SearchImage" src="phprptimages/collapse.gif" width="9" height="9" alt="" style="border: 0;"></a><span class="phpreportmaker">&nbsp;<?php echo $ReportLanguage->Phrase("Filters") ?></span>
<div id="fClickthrough_details_reportsummary_SearchPanel">
<table class="ewRptExtFilter">
	<tr id="r_user_name">
		<td><span class="ewSearchCaption"><?php echo $Page->user_name->FldCaption() ?></span></td>
		<td></td>
		<td colspan="4">
<select id="sv_user_name" name="sv_user_name"<?php echo ($Page->ClearExtFilter == "Clickthrough_details_report_user_name") ? " class=\"ewInputCleared\"" : "" ?><?php echo $Page->user_name->EditAttributes() ?>>
...
<?php
	...
	if ($cntf > 0) {
		foreach ($Page->user_name->AdvancedFilters as $filter) {
			if ($filter->Enabled) {
				$selwrk = ewr_MatchedFilterValue($Page->user_name->DropDownValue, $filter->ID) ? " selected=\"selected\"" : "";
?>
...
<?php
$sSqlWrk = "SELECT DISTINCT `user name`, `user name` AS `DispFld` FROM `vendor_click_reports`";
$sWhereWrk = "";
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
$sSqlWrk .= " ORDER BY `user name`";
?>
<input type="hidden" name="s_sv_user_name" id="s_sv_user_name" value="s=<?php echo TEAencrypt($sSqlWrk) ?>&fn=<?php echo urlencode($Page->user_name->LookupFn) ?>&f0=<?php echo TEAencrypt("`user name` = {filter_value}"); ?>&t0=200&ds=&df=0&dlm=<?php echo TEAencrypt($Page->user_name->FldDelimiter) ?>"></td>
	</tr>
	<tr id="r_click_date">
...
<input type="text" id="sv_click_date" name="sv_click_date" value="<?php echo ewr_HtmlEncode($Page->click_date->SearchValue) ?>"<?php echo ($Page->ClearExtFilter == 'Clickthrough_details_report_click_date') ? " class=\"ewInputCleared\"" : "" ?><?php echo $Page->click_date->EditAttributes() ?>>
<img src="phprptimages/calendar.png" id="fClickthrough_details_reportsummary$sv_click_date$" name="fClickthrough_details_reportsummary$sv_click_date$" alt="<?php echo $ReportLanguage->Phrase("PickDate") ?>" title="<?php echo $ReportLanguage->Phrase("PickDate") ?>" class="ewCalendar">
<script type="text/javascript">
ewr_CreateCalendar("fClickthrough_details_reportsummary", "sv_click_date", "%Y/%m/%d");
</script>
</span></td>
...
<input type="text" id="sv2_click_date" name="sv2_click_date" value="<?php echo ewr_HtmlEncode($Page->click_date->SearchValue2) ?>"<?php echo ($Page->ClearExtFilter == 'Clickthrough_details_report_click_date') ? " class=\"ewInputCleared\"" : "" ?><?php echo $Page->click_date->EditAttributes() ?>>
<img src="phprptimages/calendar.png" id="fClickthrough_details_reportsummary$sv2_click_date$" name="fClickthrough_details_reportsummary$sv2_click_date$" alt="<?php echo $ReportLanguage->Phrase("PickDate") ?>" title="<?php echo $ReportLanguage->Phrase("PickDate") ?>" class="ewCalendar">
<script type="text/javascript">
ewr_CreateCalendar("fClickthrough_details_reportsummary", "sv2_click_date", "%Y/%m/%d");
</script>
</span></td>
			</tr></table>			
		</td>
	...
	<tr id="r_np2Dname">
		...
		<td></td>
		<td colspan="4">
<select id="sv_np2Dname" name="sv_np2Dname"<?php echo ($Page->ClearExtFilter == "Clickthrough_details_report_np2Dname") ? " class=\"ewInputCleared\"" : "" ?><?php echo $Page->np2Dname->EditAttributes() ?>>
<option value=""><?php echo $ReportLanguage->Phrase("PleaseSelect") ?></option>
<?php
...
<script type="text/javascript">
fClickthrough_details_reportsummary.Init();
</script>
<!-- Search form (end) -->
<?php } ?>
...
<span<?php echo $Page->np2Dname->ViewAttributes() ?>><?php echo $Page->np2Dname->GroupViewValue ?></span></td>
<?php } ?>
<?php if ($Page->click_date->Visible) { ?>
		<td data-field="click_date"<?php echo $Page->click_date->CellAttributes(); ?>>
<span<?php echo $Page->click_date->ViewAttributes() ?>><?php echo $Page->click_date->GroupViewValue ?></span></td>
<?php } ?>
<?php if ($Page->tracking_link->Visible) { ?>
...
<?php } ?>
<?php if ($Page->tracking_link->Visible) { ?>
		<td data-field="tracking_link"<?php echo $Page->tracking_link->CellAttributes() ?>>&nbsp;</td>
<?php } ?>
<?php if ($Page->user_name->Visible) { ?>
		<td data-field="user_name"<?php echo $Page->user_name->CellAttributes() ?>>&nbsp;</td>
<?php } ?>
<?php if ($Page->Sales->Visible) { ?>
		<td data-field="Sales"<?php echo $Page->Sales->CellAttributes() ?>>
