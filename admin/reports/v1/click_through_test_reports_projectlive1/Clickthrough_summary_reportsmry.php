...
$Clickthrough_summary_report = NULL;

//
// Table class for Clickthrough summary report
//
class crClickthrough_summary_report extends crTableBase {
...
//	var $SelectLimit = TRUE;
	...
	function __construct() {
		...
		$this->TableVar = 'Clickthrough_summary_report';
		$this->TableName = 'Clickthrough summary report';
		$this->TableType = 'REPORT';
		$this->ExportAll = TRUE;
		$this->ExportPageBreakCount = 0;

		// unique id
		$this->unique_id = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_unique_id', 'unique id', '`unique id`', 200, EWR_DATATYPE_STRING, -1);
		...
		// tracking link
		$this->tracking_link = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_tracking_link', 'tracking link', '`tracking link`', 200, EWR_DATATYPE_STRING, -1);
		...
		// site user
		$this->site_user = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_site_user', 'site user', '`site user`', 200, EWR_DATATYPE_STRING, -1);
		...
		// user name
		$this->user_name = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_user_name', 'user name', '`user name`', 200, EWR_DATATYPE_STRING, -1);
		...
		// click date
		$this->click_date = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_click_date', 'click date', '`click date`', 133, EWR_DATATYPE_DATE, 5);
		...
		// vendor
		$this->vendor = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_vendor', 'vendor', '`vendor`', 200, EWR_DATATYPE_STRING, -1);
		...
		// np-id
		$this->np2Did = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_np2Did', 'np-id', '`np-id`', 200, EWR_DATATYPE_STRING, -1);
		...
		// np-name
		$this->np2Dname = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_np2Dname', 'np-name', '`np-name`', 200, EWR_DATATYPE_STRING, -1);
		...
		// Sales
		$this->Sales = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_Sales', 'Sales', '`Sales`', 5, EWR_DATATYPE_NUMBER, -1);
		...
		// Commission
		$this->Commission = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_Commission', 'Commission', '`Commission`', 5, EWR_DATATYPE_NUMBER, -1);
		...
		// pct
		$this->pct = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_pct', 'pct', '`pct`', 200, EWR_DATATYPE_STRING, -1);
		...
		// pct_giveback
		$this->pct_giveback = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_pct_giveback', 'pct_giveback', '`pct_giveback`', 200, EWR_DATATYPE_STRING, -1);
		...
		// np-share
		$this->np2Dshare = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_np2Dshare', 'np-share', '`np-share`', 5, EWR_DATATYPE_NUMBER, -1);
		...
		// bil-share
		$this->bil2Dshare = new crField('Clickthrough_summary_report', 'Clickthrough summary report', 'x_bil2Dshare', 'bil-share', '`bil-share`', 5, EWR_DATATYPE_NUMBER, -1);
	...
	function SqlSelectAgg() {
		return "SELECT SUM(`Sales`) AS `sum_sales`, SUM(`bil-share`) AS `sum_bil2dshare` FROM " . $this->SqlFrom();
	}
...
$Clickthrough_summary_report_summary = NULL; // Initialize page object first

class crClickthrough_summary_report_summary extends crClickthrough_summary_report {
	...
	// Page object name
	var $PageObjName = 'Clickthrough_summary_report_summary';
	...
	function __construct() {
		...
		// Table object (Clickthrough_summary_report)
		if (!isset($GLOBALS["Clickthrough_summary_report"])) {
			$GLOBALS["Clickthrough_summary_report"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["Clickthrough_summary_report"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'Clickthrough summary report', TRUE);
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a id=\"emf_Clickthrough_summary_report\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_Clickthrough_summary_report',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
	...
	//		$rs->MoveFirst(); // NOTE: no need to move position
		...
		if (!$rs->EOF) {
			...
			$this->Val[3] = $this->bil2Dshare->CurrentValue;
		} else {
	...
	function RenderRow() {
		...
		if ($this->RowTotalType == EWR_ROWTOTAL_GRAND && !$this->GrandSummarySetup) { // Grand total
			...
			if ($rsagg) {
				...
				$this->GrandSmry[3] = $rsagg->fields("sum_bil2dshare");
				$rsagg->Close();
				$bGotSummary = TRUE;
			}
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// bil-share
			$this->bil2Dshare->SumViewValue = $this->bil2Dshare->SumValue;
			$this->bil2Dshare->SumViewValue = ewr_FormatNumber($this->bil2Dshare->SumViewValue, $this->bil2Dshare->DefaultDecimalPrecision, -1, 0, 0);
			$this->bil2Dshare->CellAttrs["class"] =  ($this->RowTotalType == EWR_ROWTOTAL_PAGE || $this->RowTotalType == EWR_ROWTOTAL_GRAND) ? "ewRptGrpAggregate" : "ewRptGrpSummary" . $this->RowGroupLevel;
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// bil-share
			$CurrentValue = $this->bil2Dshare->SumValue;
			$ViewValue = &$this->bil2Dshare->SumViewValue;
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			if ($this->GetDropDownValue($this->np2Dname->DropDownValue, 'np2Dname')) {
				$bSetupFilter = TRUE;
			} elseif ($this->np2Dname->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_Clickthrough_summary_report_np2Dname'])) {
				$bSetupFilter = TRUE;
			}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_Clickthrough_summary_report_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_Clickthrough_summary_report_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Clickthrough_summary_report_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_Clickthrough_summary_report_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_Clickthrough_summary_report_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_Clickthrough_summary_report_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $parm) {
		$_SESSION['sv_Clickthrough_summary_report_' . $parm] = $sv;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_Clickthrough_summary_report_' . $parm] = $sv1;
		$_SESSION['so_Clickthrough_summary_report_' . $parm] = $so1;
		$_SESSION['sc_Clickthrough_summary_report_' . $parm] = $sc;
		$_SESSION['sv2_Clickthrough_summary_report_' . $parm] = $sv2;
		$_SESSION['so2_Clickthrough_summary_report_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_Clickthrough_summary_report_$parm"] = "";
		$_SESSION["rf_Clickthrough_summary_report_$parm"] = "";
		$_SESSION["rt_Clickthrough_summary_report_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->fields($parm);
		$fld->SelectionList = @$_SESSION["sel_Clickthrough_summary_report_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_Clickthrough_summary_report_$parm"];
		$fld->RangeTo = @$_SESSION["rt_Clickthrough_summary_report_$parm"];
	}
	...
	function GetSort() {
		...
		// Set up default sort
		if ($this->getOrderBy() == "") {
			$this->setOrderBy("`Sales` DESC");
			$this->Sales->setSort("DESC");
		}
...
// Create page object
if (!isset($Clickthrough_summary_report_summary)) $Clickthrough_summary_report_summary = new crClickthrough_summary_report_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$Clickthrough_summary_report_summary;
...
// Create page object
var Clickthrough_summary_report_summary = new ewr_Page("Clickthrough_summary_report_summary");

// page properties
Clickthrough_summary_report_summary.PageID = "summary"; // page ID
var EWR_PAGE_ID = Clickthrough_summary_report_summary.PageID;

// extend page with Chart_Rendering function
Clickthrough_summary_report_summary.Chart_Rendering =  
...
Clickthrough_summary_report_summary.Chart_Rendered =  
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var fClickthrough_summary_reportsummary = new ewr_Form("fClickthrough_summary_reportsummary");

// Validate method
fClickthrough_summary_reportsummary.Validate = function(fobj) {
...
fClickthrough_summary_reportsummary.Form_CustomValidate =  
...
fClickthrough_summary_reportsummary.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
fClickthrough_summary_reportsummary.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// Use Ajax
fClickthrough_summary_reportsummary.Lists["sv_np2Dname"] = {"LinkField":"sv_np2Dname","DisplayFields":["sv_np2Dname","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
</script>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fClickthrough_summary_reportsummary" id="fClickthrough_summary_reportsummary" class="ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>" onsubmit="return ewrForms[this.id].Submit();">
<input type="hidden" name="cmd" value="search">
<a href="javascript:fClickthrough_summary_reportsummary.ToggleSearchPanel();" style="text-decoration: none;"><img id="fClickthrough_summary_reportsummary_SearchImage" src="phprptimages/collapse.gif" width="9" height="9" alt="" style="border: 0;"></a><span class="phpreportmaker">&nbsp;<?php echo $ReportLanguage->Phrase("Filters") ?></span>
<div id="fClickthrough_summary_reportsummary_SearchPanel">
...
<input type="text" id="sv_click_date" name="sv_click_date" value="<?php echo ewr_HtmlEncode($Page->click_date->SearchValue) ?>"<?php echo ($Page->ClearExtFilter == 'Clickthrough_summary_report_click_date') ? " class=\"ewInputCleared\"" : "" ?><?php echo $Page->click_date->EditAttributes() ?>>
<img src="phprptimages/calendar.png" id="fClickthrough_summary_reportsummary$sv_click_date$" name="fClickthrough_summary_reportsummary$sv_click_date$" alt="<?php echo $ReportLanguage->Phrase("PickDate") ?>" title="<?php echo $ReportLanguage->Phrase("PickDate") ?>" class="ewCalendar">
<script type="text/javascript">
ewr_CreateCalendar("fClickthrough_summary_reportsummary", "sv_click_date", "%Y/%m/%d");
</script>
</span></td>
...
<input type="text" id="sv2_click_date" name="sv2_click_date" value="<?php echo ewr_HtmlEncode($Page->click_date->SearchValue2) ?>"<?php echo ($Page->ClearExtFilter == 'Clickthrough_summary_report_click_date') ? " class=\"ewInputCleared\"" : "" ?><?php echo $Page->click_date->EditAttributes() ?>>
<img src="phprptimages/calendar.png" id="fClickthrough_summary_reportsummary$sv2_click_date$" name="fClickthrough_summary_reportsummary$sv2_click_date$" alt="<?php echo $ReportLanguage->Phrase("PickDate") ?>" title="<?php echo $ReportLanguage->Phrase("PickDate") ?>" class="ewCalendar">
<script type="text/javascript">
ewr_CreateCalendar("fClickthrough_summary_reportsummary", "sv2_click_date", "%Y/%m/%d");
</script>
</span></td>
			</tr></table>			
		</td>
	...
	<tr id="r_np2Dname">
		...
		<td></td>
		<td colspan="4">
<select id="sv_np2Dname" name="sv_np2Dname"<?php echo ($Page->ClearExtFilter == "Clickthrough_summary_report_np2Dname") ? " class=\"ewInputCleared\"" : "" ?><?php echo $Page->np2Dname->EditAttributes() ?>>
...
<script type="text/javascript">
fClickthrough_summary_reportsummary.Init();
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php if ($Page->GrpFldCount + $Page->DtlFldCount > 0) { ?>
		<td colspan="<?php echo ($Page->GrpFldCount + $Page->DtlFldCount) ?>"<?php echo $Page->np2Dname->CellAttributes() ?>><?php echo $ReportLanguage->Phrase("RptSumHead") ?> <?php echo $Page->np2Dname->FldCaption() ?>: <?php echo $Page->np2Dname->GroupViewValue ?> (<?php echo ewr_FormatNumber($Page->Cnt[1][0],0,-2,-2,-2) ?><?php echo $ReportLanguage->Phrase("RptDtlRec") ?>)</td>
<?php } ?>
</tr>
<?php
			$Page->ResetAttrs();
			...
			$Page->bil2Dshare->Count = $Page->Cnt[1][3];
			$Page->bil2Dshare->SumValue = $Page->Smry[1][3]; // Load SUM
			$Page->RowTotalSubType = EWR_ROWTOTAL_SUM;
			$Page->RenderRow();
?>
	<tr<?php echo $Page->RowAttributes(); ?>>
		<td colspan="<?php echo ($Page->GrpFldCount - 0) ?>"<?php echo $Page->np2Dname->CellAttributes() ?>><?php echo $ReportLanguage->Phrase("RptSum"); ?></td>
<?php if ($Page->Sales->Visible) { ?>
		<td data-field="Sales"<?php echo $Page->bil2Dshare->CellAttributes() ?>>
<span<?php echo $Page->Sales->ViewAttributes() ?>><?php echo $Page->Sales->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->pct_giveback->Visible) { ?>
		<td data-field="pct_giveback"<?php echo $Page->np2Dname->CellAttributes() ?>>&nbsp;</td>
<?php } ?>
<?php if ($Page->bil2Dshare->Visible) { ?>
		<td data-field="bil2Dshare"<?php echo $Page->bil2Dshare->CellAttributes() ?>>
<span<?php echo $Page->bil2Dshare->ViewAttributes() ?>><?php echo $Page->bil2Dshare->SumViewValue ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php
	...
	$Page->RowTotalSubType = EWR_ROWTOTAL_SUM;
	$Page->bil2Dshare->Count = $Page->GrandCnt[3];
	$Page->bil2Dshare->SumValue = $Page->GrandSmry[3]; // Load SUM
	$Page->RowTotalSubType = EWR_ROWTOTAL_SUM;
	$Page->RowAttrs["class"] = "ewRptGrandSummary";
	$Page->RenderRow();
?>
...
<?php } ?>
<?php if ($Page->pct_giveback->Visible) { ?>
		<td data-field="pct_giveback"<?php echo $Page->pct_giveback->CellAttributes() ?>>&nbsp;</td>
<?php } ?>
<?php if ($Page->bil2Dshare->Visible) { ?>
		<td data-field="bil2Dshare"<?php echo $Page->bil2Dshare->CellAttributes() ?>>
<span<?php echo $Page->bil2Dshare->ViewAttributes() ?>><?php echo $Page->bil2Dshare->SumViewValue ?></span></td>
