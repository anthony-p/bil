...
//
// Table class for sales by month
//
class crsales_by_month extends crTableBase {
...
//	var $SelectLimit = TRUE;
	...
	function __construct() {
		...
		$this->TableVar = 'sales_by_month';
		$this->TableName = 'sales by month';
		$this->TableType = 'REPORT';
		$this->ExportAll = TRUE;
		$this->ExportPageBreakCount = 0;

		// unique id
		$this->unique_id = new crField('sales_by_month', 'sales by month', 'x_unique_id', 'unique id', '`unique id`', 200, EWR_DATATYPE_STRING, -1);
		...
		// tracking link
		$this->tracking_link = new crField('sales_by_month', 'sales by month', 'x_tracking_link', 'tracking link', '`tracking link`', 200, EWR_DATATYPE_STRING, -1);
		...
		// site user
		$this->site_user = new crField('sales_by_month', 'sales by month', 'x_site_user', 'site user', '`site user`', 200, EWR_DATATYPE_STRING, -1);
		...
		// user name
		$this->user_name = new crField('sales_by_month', 'sales by month', 'x_user_name', 'user name', '`user name`', 200, EWR_DATATYPE_STRING, -1);
		...
		// click date
		$this->click_date = new crField('sales_by_month', 'sales by month', 'x_click_date', 'click date', '`click date`', 133, EWR_DATATYPE_DATE, 5);
		$this->click_date->GroupingFieldId = 1;
		...
		$this->click_date->SqlSelect = "SELECT DISTINCT `click date`, CONCAT(CAST(YEAR(`click date`) AS CHAR(4)), '|', CAST(LPAD(MONTH(`click date`),2,'0') AS CHAR(2))) AS `ew_report_groupvalue` FROM " . $this->SqlFrom();
		$this->click_date->SqlOrderBy = "CONCAT(CAST(YEAR(`click date`) AS CHAR(4)), '|', CAST(LPAD(MONTH(`click date`),2,'0') AS CHAR(2)))";
		$this->click_date->FldGroupByType = "m";
		$this->click_date->FldGroupInt = "0";
		$this->click_date->FldGroupSql = "CONCAT(CAST(YEAR(%s) AS CHAR(4)), '|', CAST(LPAD(MONTH(%s),2,'0') AS CHAR(2)))";

		// vendor
		$this->vendor = new crField('sales_by_month', 'sales by month', 'x_vendor', 'vendor', '`vendor`', 200, EWR_DATATYPE_STRING, -1);
		...
		// np-id
		$this->np2Did = new crField('sales_by_month', 'sales by month', 'x_np2Did', 'np-id', '`np-id`', 200, EWR_DATATYPE_STRING, -1);
		...
		// np-name
		$this->np2Dname = new crField('sales_by_month', 'sales by month', 'x_np2Dname', 'np-name', '`np-name`', 200, EWR_DATATYPE_STRING, -1);
		$this->np2Dname->GroupingFieldId = 2;
		...
		// Sales
		$this->Sales = new crField('sales_by_month', 'sales by month', 'x_Sales', 'Sales', '`Sales`', 5, EWR_DATATYPE_NUMBER, -1);
		...
		// Commission
		$this->Commission = new crField('sales_by_month', 'sales by month', 'x_Commission', 'Commission', '`Commission`', 5, EWR_DATATYPE_NUMBER, -1);
		...
		// pct
		$this->pct = new crField('sales_by_month', 'sales by month', 'x_pct', 'pct', '`pct`', 200, EWR_DATATYPE_STRING, -1);
		...
		// pct_giveback
		$this->pct_giveback = new crField('sales_by_month', 'sales by month', 'x_pct_giveback', 'pct_giveback', '`pct_giveback`', 200, EWR_DATATYPE_STRING, -1);
		...
		// np-share
		$this->np2Dshare = new crField('sales_by_month', 'sales by month', 'x_np2Dshare', 'np-share', '`np-share`', 5, EWR_DATATYPE_NUMBER, -1);
		...
		// bil-share
		$this->bil2Dshare = new crField('sales_by_month', 'sales by month', 'x_bil2Dshare', 'bil-share', '`bil-share`', 5, EWR_DATATYPE_NUMBER, -1);
		...
		// sales by month
		$this->sales_by_month = new crChart('sales_by_month', 'sales by month', 'sales_by_month', 'sales by month', 'click date', 'Sales', 'np-name', 10, 'SUM', 550, 440);
		$this->sales_by_month->SqlSelect = "SELECT `click date`, `np-name`, SUM(`Sales`) FROM ";
		$this->sales_by_month->SqlGroupBy = "`click date`, `np-name`";
		$this->sales_by_month->SqlOrderBy = "";
		$this->sales_by_month->SeriesDateType = "";
		$this->sales_by_month->SqlSelectSeries = "SELECT DISTINCT `np-name` FROM ";
		$this->sales_by_month->SqlGroupBySeries = "`np-name`";
		$this->sales_by_month->SqlOrderBySeries = "";
		$this->sales_by_month->XAxisDateFormat = 5;
	}
	...
	function SqlSelect() { // Select
		return "SELECT *, CONCAT(CAST(YEAR(`click date`) AS CHAR(4)), '|', CAST(LPAD(MONTH(`click date`),2,'0') AS CHAR(2))) FROM " . $this->SqlFrom();
	}
	...
	function SqlOrderBy() { // Order By
		return "CONCAT(CAST(YEAR(`click date`) AS CHAR(4)), '|', CAST(LPAD(MONTH(`click date`),2,'0') AS CHAR(2))) ASC, `np-name` ASC";
	}

	// Table Level Group SQL
	function SqlFirstGroupField() {
		return "CONCAT(CAST(YEAR(`click date`) AS CHAR(4)), '|', CAST(LPAD(MONTH(`click date`),2,'0') AS CHAR(2)))";
	}
	...
	function SqlOrderByGroup() {
		return "CONCAT(CAST(YEAR(`click date`) AS CHAR(4)), '|', CAST(LPAD(MONTH(`click date`),2,'0') AS CHAR(2))) ASC";
	}
...
$sales_by_month_summary = NULL; // Initialize page object first

class crsales_by_month_summary extends crsales_by_month {
	...
	// Page object name
	var $PageObjName = 'sales_by_month_summary';
	...
	function __construct() {
		...
		// Table object (sales_by_month)
		if (!isset($GLOBALS["sales_by_month"])) {
			$GLOBALS["sales_by_month"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["sales_by_month"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'sales by month', TRUE);
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a id=\"emf_sales_by_month\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_sales_by_month',hdr:ewLanguage.Phrase('ExportToEmail')});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
	...
	function Page_Main() {
		...
		$this->Col = array(array(FALSE, FALSE), array(TRUE,FALSE), array(FALSE,FALSE), array(TRUE,FALSE));
		...
		switch ($lvl) {
			case 1:
				return (is_null($this->click_date->CurrentValue) && !is_null($this->click_date->OldValue)) ||
					...
					($this->click_date->GroupValue() <> $this->click_date->GroupOldValue());
			case 2:
				return (is_null($this->np2Dname->CurrentValue) && !is_null($this->np2Dname->OldValue)) ||
					...
					($this->np2Dname->GroupValue() <> $this->np2Dname->GroupOldValue()) || $this->ChkLvlBreak(1); // Recurse upper level
		}
	}
	...
	function GetGrpRow($opt) {
		...
		if ($opt == 1) { // Get first group
			...
			$this->click_date->setDbValue(""); // Init first value
		} else { // Get next group
			$rsgrp->MoveNext();
		}
		if (!$rsgrp->EOF)
			$this->click_date->setDbValue($rsgrp->fields[0]);
		if ($rsgrp->EOF) {
	...
	//		$rs->MoveFirst(); // NOTE: no need to move position
		...
		if (!$rs->EOF) {
			...
			if ($opt <> 1) {
				if (is_array($this->click_date->GroupDbValues))
					$this->click_date->setDbValue(@$this->click_date->GroupDbValues[$rs->fields('click date')]);
				else
					$this->click_date->setDbValue(ewr_GroupValue($this->click_date, $rs->fields('click date')));
			}
			...
			$this->Val[1] = $this->Sales->CurrentValue;
			$this->Val[2] = $this->pct_giveback->CurrentValue;
			$this->Val[3] = $this->np2Dshare->CurrentValue;
		} else {
	...
	function LoadGroupDbValues() {
		...
		// Set up click date GroupDbValues
		$sSql = ewr_BuildReportSql($this->click_date->SqlSelect, $this->SqlWhere(), $this->SqlGroupBy(), $this->SqlHaving(), $this->click_date->SqlOrderBy, "", "");
		$rswrk = $conn->Execute($sSql);
		while ($rswrk && !$rswrk->EOF) {
			$this->click_date->setDbValue($rswrk->fields[0]);
			if (!is_null($this->click_date->CurrentValue) && $this->click_date->CurrentValue <> "") {
				$grpval = $rswrk->fields('ew_report_groupvalue');
				$this->click_date->GroupDbValues[$this->click_date->CurrentValue] = $grpval;
			}
			$rswrk->MoveNext();
		}
		if ($rswrk)
	...
	function RenderRow() {
		...
		if ($this->RowTotalType == EWR_ROWTOTAL_GRAND && !$this->GrandSummarySetup) { // Grand total
			...
			if ($rsagg) {
				...
				$this->GrandSmry[1] = $rsagg->fields("sum_sales");
				$this->GrandCnt[2] = $this->TotCount;
				$this->GrandCnt[3] = $this->TotCount;
				$this->GrandSmry[3] = $rsagg->fields("sum_np2dshare");
				$rsagg->Close();
				$bGotSummary = TRUE;
			}
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			$this->click_date->CellAttrs["class"] = ($this->RowGroupLevel == 1) ? "ewRptGrpSummary1" : "ewRptGrpField1";
			...
			$this->np2Dname->CellAttrs["class"] = ($this->RowGroupLevel == 2) ? "ewRptGrpSummary2" : "ewRptGrpField2";
			...
			$this->Sales->CellAttrs["class"] = ($this->RowTotalType == EWR_ROWTOTAL_PAGE || $this->RowTotalType == EWR_ROWTOTAL_GRAND) ? "ewRptGrpAggregate" : (($this->RowGroupLevel < 2) ? "ewRptGrpSummary" . $this->RowGroupLevel : (($this->LastGrpCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow"));
			...
			$this->np2Dshare->CellAttrs["class"] = ($this->RowTotalType == EWR_ROWTOTAL_PAGE || $this->RowTotalType == EWR_ROWTOTAL_GRAND) ? "ewRptGrpAggregate" : (($this->RowGroupLevel < 2) ? "ewRptGrpSummary" . $this->RowGroupLevel : (($this->LastGrpCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow"));
			...
			$this->click_date->CellAttrs["class"] = "ewRptGrpField1";
			$this->click_date->GroupViewValue = ewr_DisplayGroupValue($this->click_date, $this->click_date->GroupViewValue);
			if ($this->click_date->GroupValue() == $this->click_date->GroupOldValue() && !$this->ChkLvlBreak(1))
				$this->click_date->GroupViewValue = "&nbsp;";

			// np-name
			$this->np2Dname->GroupViewValue = $this->np2Dname->GroupValue();
			$this->np2Dname->CellAttrs["class"] = "ewRptGrpField2";
			$this->np2Dname->GroupViewValue = ewr_DisplayGroupValue($this->np2Dname, $this->np2Dname->GroupViewValue);
			if ($this->np2Dname->GroupValue() == $this->np2Dname->GroupOldValue() && !$this->ChkLvlBreak(2))
	...
	function SetupFieldCount() {
		...
		$this->DtlFldCount = 0;
		if ($this->click_date->Visible) $this->GrpFldCount += 1;
		if ($this->np2Dname->Visible) { $this->GrpFldCount += 1; $this->SubGrpFldCount += 1; }
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			if ($this->GetDropDownValue($this->np2Dname->DropDownValue, 'np2Dname')) {
				$bSetupFilter = TRUE;
			} elseif ($this->np2Dname->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_sales_by_month_np2Dname'])) {
				$bSetupFilter = TRUE;
			}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_sales_by_month_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_sales_by_month_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_sales_by_month_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_sales_by_month_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_sales_by_month_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_sales_by_month_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $parm) {
		$_SESSION['sv_sales_by_month_' . $parm] = $sv;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_sales_by_month_' . $parm] = $sv1;
		$_SESSION['so_sales_by_month_' . $parm] = $so1;
		$_SESSION['sc_sales_by_month_' . $parm] = $sc;
		$_SESSION['sv2_sales_by_month_' . $parm] = $sv2;
		$_SESSION['so2_sales_by_month_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_sales_by_month_$parm"] = "";
		$_SESSION["rf_sales_by_month_$parm"] = "";
		$_SESSION["rt_sales_by_month_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->fields($parm);
		$fld->SelectionList = @$_SESSION["sel_sales_by_month_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_sales_by_month_$parm"];
		$fld->RangeTo = @$_SESSION["rt_sales_by_month_$parm"];
	}
...
// Create page object
if (!isset($sales_by_month_summary)) $sales_by_month_summary = new crsales_by_month_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$sales_by_month_summary;
...
// Create page object
var sales_by_month_summary = new ewr_Page("sales_by_month_summary");

// page properties
sales_by_month_summary.PageID = "summary"; // page ID
var EWR_PAGE_ID = sales_by_month_summary.PageID;

// extend page with Chart_Rendering function
sales_by_month_summary.Chart_Rendering =  
...
sales_by_month_summary.Chart_Rendered =  
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var fsales_by_monthsummary = new ewr_Form("fsales_by_monthsummary");

// Validate method
fsales_by_monthsummary.Validate = function(fobj) {
...
fsales_by_monthsummary.Form_CustomValidate =  
...
fsales_by_monthsummary.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
fsales_by_monthsummary.ValidateRequired = false; // no JavaScript validation
<?php } ?>

// Use Ajax
fsales_by_monthsummary.Lists["sv_np2Dname"] = {"LinkField":"sv_np2Dname","DisplayFields":["sv_np2Dname","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
</script>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fsales_by_monthsummary" id="fsales_by_monthsummary" class="ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>" onsubmit="return ewrForms[this.id].Submit();">
<input type="hidden" name="cmd" value="search">
<a href="javascript:fsales_by_monthsummary.ToggleSearchPanel();" style="text-decoration: none;"><img id="fsales_by_monthsummary_SearchImage" src="phprptimages/collapse.gif" width="9" height="9" alt="" style="border: 0;"></a><span class="phpreportmaker">&nbsp;<?php echo $ReportLanguage->Phrase("Filters") ?></span>
<div id="fsales_by_monthsummary_SearchPanel">
...
<input type="text" id="sv_click_date" name="sv_click_date" value="<?php echo ewr_HtmlEncode($Page->click_date->SearchValue) ?>"<?php echo ($Page->ClearExtFilter == 'sales_by_month_click_date') ? " class=\"ewInputCleared\"" : "" ?><?php echo $Page->click_date->EditAttributes() ?>>
<img src="phprptimages/calendar.png" id="fsales_by_monthsummary$sv_click_date$" name="fsales_by_monthsummary$sv_click_date$" alt="<?php echo $ReportLanguage->Phrase("PickDate") ?>" title="<?php echo $ReportLanguage->Phrase("PickDate") ?>" class="ewCalendar">
<script type="text/javascript">
ewr_CreateCalendar("fsales_by_monthsummary", "sv_click_date", "%Y/%m/%d");
</script>
</span></td>
...
<input type="text" id="sv2_click_date" name="sv2_click_date" value="<?php echo ewr_HtmlEncode($Page->click_date->SearchValue2) ?>"<?php echo ($Page->ClearExtFilter == 'sales_by_month_click_date') ? " class=\"ewInputCleared\"" : "" ?><?php echo $Page->click_date->EditAttributes() ?>>
<img src="phprptimages/calendar.png" id="fsales_by_monthsummary$sv2_click_date$" name="fsales_by_monthsummary$sv2_click_date$" alt="<?php echo $ReportLanguage->Phrase("PickDate") ?>" title="<?php echo $ReportLanguage->Phrase("PickDate") ?>" class="ewCalendar">
<script type="text/javascript">
ewr_CreateCalendar("fsales_by_monthsummary", "sv2_click_date", "%Y/%m/%d");
</script>
</span></td>
			</tr></table>			
		</td>
	...
	<tr id="r_np2Dname">
		...
		<td></td>
		<td colspan="4">
<select id="sv_np2Dname" name="sv_np2Dname"<?php echo ($Page->ClearExtFilter == "sales_by_month_np2Dname") ? " class=\"ewInputCleared\"" : "" ?><?php echo $Page->np2Dname->EditAttributes() ?>>
...
<?php if ($Page->TotalGrps > 0) { ?>
	<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
	<td><table cellspacing="0" class="ewStdTable"><tbody><tr><td><?php echo $ReportLanguage->Phrase("GroupsPerPage") ?>&nbsp;
...
<td data-field="Sales" class="ewTableHeader">
	<table class="ewTableHeaderBtn"><tr><td><?php echo $Page->Sales->FldCaption() ?> (<?php echo $ReportLanguage->Phrase("RptSum"); ?>)</td></tr></table>
</td>
<?php } ?>
<?php if ($Page->np2Dshare->Visible) { ?>
<td data-field="np2Dshare" class="ewTableHeader">
	<table class="ewTableHeaderBtn"><tr><td><?php echo $Page->np2Dshare->FldCaption() ?> (<?php echo $ReportLanguage->Phrase("RptSum"); ?>)</td></tr></table>
</td>
<?php } ?>
	</tr>
</thead>
...
<?php
	...
	// Build detail SQL
	$sWhere = ewr_DetailFilterSQL($Page->click_date, $Page->SqlFirstGroupField(), $Page->click_date->GroupValue());
...
?>
<?php
		if ($Page->ChkLvlBreak(2) && $Page->np2Dname->Visible) {
			$Page->LastGrpCount++; // Update last group count
?>
<?php
			$Page->Sales->Count = $Page->Cnt[2][1];
			$Page->Sales->SumValue = $Page->Smry[2][1]; // Load SUM
			$Page->pct_giveback->Count = $Page->Cnt[2][2];
			$Page->np2Dshare->Count = $Page->Cnt[2][3];
			$Page->np2Dshare->SumValue = $Page->Smry[2][3]; // Load SUM
			$Page->ResetAttrs();
...
<span<?php echo $Page->click_date->ViewAttributes() ?>><?php echo $Page->click_date->GroupSummaryViewValue ?></span></td>
<?php } ?>
<?php if ($Page->np2Dname->Visible) { ?>
		<td data-field="np2Dname"<?php echo $Page->np2Dname->CellAttributes() ?>>
<span<?php echo $Page->np2Dname->ViewAttributes() ?>><?php echo $Page->np2Dname->GroupViewValue ?></span>&nbsp;<span class="ewDetailCount">(<?php echo ewr_FormatNumber($Page->Cnt[2][0],0,-2,-2,-2); ?><?php echo $ReportLanguage->Phrase("RptDtlRec") ?>)</span></td>
<?php } ?>
<?php if ($Page->Sales->Visible) { ?>
		<td data-field="Sales"<?php echo $Page->Sales->CellAttributes() ?>>
<span<?php echo $Page->Sales->ViewAttributes() ?>><?php echo $Page->Sales->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->np2Dshare->Visible) { ?>
		<td data-field="np2Dshare"<?php echo $Page->np2Dshare->CellAttributes() ?>>
<span<?php echo $Page->np2Dshare->ViewAttributes() ?>><?php echo $Page->np2Dshare->SumViewValue ?></span></td>
<?php } ?>
	</tr>
<?php
...
?>
<?php
			$Page->Sales->Count = $Page->Cnt[1][1];
			$Page->Sales->SumValue = $Page->Smry[1][1]; // Load SUM
			$Page->pct_giveback->Count = $Page->Cnt[1][2];
			$Page->np2Dshare->Count = $Page->Cnt[1][3];
			$Page->np2Dshare->SumValue = $Page->Smry[1][3]; // Load SUM
			$Page->ResetAttrs();
			$Page->RowType = EWR_ROWTYPE_TOTAL;
			$Page->RowTotalType = EWR_ROWTOTAL_GROUP;
			$Page->RowTotalSubType = EWR_ROWTOTAL_FOOTER;
...
<?php if ($Page->click_date->Visible) { ?>
		<td data-field="click_date"<?php echo $Page->click_date->CellAttributes() ?>>
<span<?php echo $Page->click_date->ViewAttributes() ?>><?php echo $Page->click_date->GroupViewValue ?></span>&nbsp;<span class="ewDetailCount">(<?php echo ewr_FormatNumber($Page->Cnt[1][0],0,-2,-2,-2); ?><?php echo $ReportLanguage->Phrase("RptDtlRec") ?>)</span></td>
<?php } ?>
<?php if ($Page->np2Dname->Visible) { ?>
		<td data-field="np2Dname"<?php echo $Page->click_date->CellAttributes() ?>>&nbsp;</td>
<?php } ?>
<?php if ($Page->Sales->Visible) { ?>
		<td data-field="Sales"<?php echo $Page->Sales->CellAttributes() ?>>
<span<?php echo $Page->Sales->ViewAttributes() ?>><?php echo $Page->Sales->SumViewValue ?></span></td>
...
?>
<?php
	$Page->Sales->Count = $Page->GrandCnt[1];
	$Page->Sales->SumValue = $Page->GrandSmry[1]; // Load SUM
	$Page->pct_giveback->Count = $Page->GrandCnt[2];
	$Page->np2Dshare->Count = $Page->GrandCnt[3];
	$Page->np2Dshare->SumValue = $Page->GrandSmry[3]; // Load SUM
	$Page->ResetAttrs();
	$Page->RowType = EWR_ROWTYPE_TOTAL;
	$Page->RowTotalType = EWR_ROWTOTAL_GRAND;
	$Page->RowTotalSubType = EWR_ROWTOTAL_FOOTER;
...
?>
	<tr<?php echo $Page->RowAttributes(); ?>>
		<td><?php echo $ReportLanguage->Phrase("RptGrandTotal") ?>&nbsp;<span class="ewDetailCount">(<?php echo ewr_FormatNumber($Page->TotCount,0,-2,-2,-2); ?><?php echo $ReportLanguage->Phrase("RptDtlRec") ?>)</span></td>
<?php if ($Page->np2Dname->Visible) { ?>
		<td data-field="np2Dshare">&nbsp;</td>
<?php } ?>
<?php if ($Page->Sales->Visible) { ?>
		<td data-field="Sales"<?php echo $Page->Sales->CellAttributes() ?>>
<span<?php echo $Page->Sales->ViewAttributes() ?>><?php echo $Page->Sales->SumViewValue ?></span></td>
...
<?php } ?>
	<!-- Bottom slot -->
<?php if ($Page->Export == "print" && $Page->ExportChartPageBreak) { ?>
<?php echo $Page->PageBreakContent ?>
<?php } ?>
<?php if ($Page->Export == "" || $Page->Export == "print" || $Page->Export == "email") { ?>
<?php

// Initialize chart data
$Page->sales_by_month->ID = "sales_by_month_sales_by_month"; // Chart ID
$Page->sales_by_month->SetChartParms(array(array("type", "10", FALSE),
	array("seriestype", "0", FALSE)));  // Chart type / Chart series type
$Page->sales_by_month->SetChartParm("bgcolor", "FCFCFC", TRUE); // Background color
$Page->sales_by_month->SetChartParms(array(array("caption", $Page->sales_by_month->ChartCaption()),
	array("xaxisname", $Page->sales_by_month->ChartXAxisName()))); // Chart caption / X axis name
$Page->sales_by_month->SetChartParm("yaxisname", $Page->sales_by_month->ChartYAxisName(), TRUE); // Y axis name
$Page->sales_by_month->SetChartParms(array(array("shownames", "0"),
	array("showvalues", "0"),
	array("showhovercap", "1"))); // Show names / Show values / Show hover
$Page->sales_by_month->SetChartParm("alpha", "50", FALSE); // Chart alpha
$Page->sales_by_month->SetChartParm("colorpalette", "#FF0000|#FF0080|#FF00FF|#8000FF|#FF8000|#FF3D3D|#7AFFFF|#0000FF|#FFFF00|#FF7A7A|#3DFFFF|#0080FF|#80FF00|#00FF00|#00FF80|#00FFFF", FALSE); // Chart color palette
?>
<?php
$Page->sales_by_month->SetChartParms(array(array("showCanvasBg", "1"),
...
// Define trend lines
?>
<a id="cht_sales_by_month"></a>
<table class="ewStdTable"><tr><td><div id="div_ctl_sales_by_month_sales_by_month" class="ewChart">
<div id="div_sales_by_month_sales_by_month"></div>
</div></td></tr></table>
<?php
$SqlSelect = $Page->SqlSelect();
$SqlChartSelect = $Page->sales_by_month->SqlSelect;
$sSqlChartBase = $Page->SqlFrom();

// Load chart series from sql directly
$sSql = $Page->sales_by_month->SqlSelectSeries . $sSqlChartBase;
$sSql = ewr_BuildReportSql($sSql, $Page->SqlWhere(), $Page->sales_by_month->SqlGroupBySeries, "", $Page->sales_by_month->SqlOrderBySeries, $Page->Filter, "");
if (EWR_DEBUG_ENABLED) echo "(Chart Series SQL): " . $sSql . "<div class=\"ewSpacer\">&nbsp;</div>";
ewr_LoadChartSeries($sSql, $Page->sales_by_month);

// Load chart data from sql directly
$sSql = $SqlChartSelect . $sSqlChartBase;
$sSql = ewr_BuildReportSql($sSql, $Page->SqlWhere(), $Page->sales_by_month->SqlGroupBy, "", $Page->sales_by_month->SqlOrderBy, $Page->Filter, "");
if (EWR_DEBUG_ENABLED) echo "(Chart SQL): " . $sSql . "<div class=\"ewSpacer\">&nbsp;</div>";
ewr_LoadChartData($sSql, $Page->sales_by_month);
ewr_SortMultiChartData($Page->sales_by_month->Data, 0, "");

// Render chart
$chartxml = $Page->RenderChart($Page->sales_by_month);
echo $Page->sales_by_month->ShowChartFCF($chartxml);
?>
<?php if ($Page->Export <> "email" && !$Page->DrillDown) { ?>
<a href="javascript:void(0);" class="ewTopLink" onclick="ewr_Select('#top')[0].scrollIntoView();"><?php echo $ReportLanguage->Phrase("Top") ?></a>
<?php } ?>
<?php } ?>
<?php if ($Page->Export == "") { ?>
	</div></td></tr>
