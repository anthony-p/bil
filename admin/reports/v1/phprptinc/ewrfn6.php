...
// Functions for PHP Report Maker 6
// (C) 2007-2012 e.World Technology Limited
...
function CurrentProjectID() {
	...
	return "{1970966F-5C22-4F67-9D47-2A354E499C8D}";
}
...
class crLanguage {
	...
	function Load($id) {
		...
		if ($bUseSystemLocale) {
			...
			if (!empty($decimal_point))
				$EWR_DEFAULT_DECIMAL_POINT = $decimal_point;
			$EWR_DEFAULT_THOUSANDS_SEP = $thousands_sep;
			if (!empty($mon_decimal_point))
				$EWR_DEFAULT_MON_DECIMAL_POINT = $mon_decimal_point;
			$EWR_DEFAULT_MON_THOUSANDS_SEP = $mon_thousands_sep;
	...
	function Phrase($Name) {
		if (is_object($this->Phrases)) {
			...
			return ewr_ConvertFromUtf8(@$this->Phrases['ew-language']['global']['phrase'][strtolower($Name)]['attr']['value']);
		}
	}
...
class crTimer {
	...
	function Stop() {
		...
		if (isset($this->EndTime) && isset($this->StartTime) &&
			...
			echo '<div class="ewSpacer">Page processing time: ' . ($this->EndTime - $this->StartTime) . ' seconds</div>';
	}
}
...
class crTableBase {
	...
	var $PageBreakContent = "<div class=\"ewPageBreak\">&nbsp;</div>"; // Page break content
...
class crMenu {
	...
	function AddMenuItem($id, $text, $url, $parentid = -1, $src = "", $allowed = TRUE, $grouptitle = FALSE) {
		$item = new crMenuItem($id, $text, $url, $parentid, $src, $allowed, $grouptitle);
	...
	function Render($ret = FALSE) {
		...
		if (!$this->IsMobile) {
			...
			$str .= " class=\"" . ($this->IsRoot ? EWR_MENUBAR_CLASSNAME : EWR_MENU_CLASSNAME) . "\">";
		...
		foreach ($this->ItemData as $item) {
			if ($this->RenderItem($item)) {
				...
				$aclass = $this->IsRoot ? EWR_MENUBAR_ITEM_LABEL_CLASSNAME : EWR_MENU_ITEM_LABEL_CLASSNAME;
				$liclass = $this->IsRoot ? EWR_MENUBAR_ITEM_CLASSNAME : EWR_MENU_ITEM_CLASSNAME;
				if ($item->GroupTitle && EWR_MENU_ITEM_CLASSNAME <> "") { // Group title
					$gcnt++;
					if ($i > 1 && $gopen) {
						if (!$this->IsMobile)
							$str .= "</ul>\n"; // End last group
...
class crMenuItem {
	...
	function AddItem($item, $mobile = FALSE) { // Add submenu item
	...
	function Render($aclass = "", $liclass = "", $mobile = FALSE) {
		...
		$innerhtml = ewr_HtmlElement("a", $attrs, $this->Text);
		if (!is_null($this->SubMenu)) {
			if ($mobile && $this->Url <> "")
				$innerhtml .= $innerhtml;
...
class crListOption {
	...
	function Render($Part, $ColSpan = 1) {
		...
		if (strval($value) == "" && strtolower($this->Parent->Tag) <> "td")
			return "";
		$res = ($value <> "") ? $value : "&nbsp;";
...
class crAdvancedSecurity {
	...
	// Add User Level by name (for use with UserLevel_Loaded)
	...
	// Add User Level by ID (for use with UserLevel_Loaded)
	...
	// Delete User Level by name (for use with UserLevel_Loaded)
	...
	// Delete User Level by ID (for use with UserLevel_Loaded)
	...
	function ShowUserLevelInfo() {
		...
		echo "</pre>";
		echo "<div class=\"ewSpacer\">Current User Level ID = " . $this->CurrentUserLevelID() . "</div>";
		echo "<div class=\"ewSpacer\">Current User Level ID List = " . $this->UserLevelList() . "</div>";
	}
...
// Load recordset (for compatibility project)
function &ew_LoadRecordset($SQL) {
	return ewr_LoadRecordset($SQL);
}
...
// Execute UPDATE, INSERT, or DELETE statements (for compatibility project)
function ew_Execute($SQL) {
	return ewr_Execute($SQL);
}
...
// Executes the query, and returns the first column of the first row (for compatibility project)
function ew_ExecuteScalar($SQL) {
	return ewr_ExecuteScalar($SQL);
}
...
// Executes the query, and returns the first row (for compatibility project)
function ew_ExecuteRow($SQL) {
	return ewr_ExecuteRow($SQL);
}
...
// Add message
function ewr_AddMessage(&$msg, $msgtoadd, $sep = "<div class=\"ewSpacer\">&nbsp;</div>") {
	if (strval($msgtoadd) <> "") {
		if (strval($msg) <> "")
			$msg .= $sep;
		$msg .= $msgtoadd;
...
class crEmail {
    ...
    var $pdfAttachmentFileName="";
    var $pdfAttachmentContent="";
	...
	function Send() {
		...
		$result = ewr_SendEmail($this->Sender, $this->Recipient, $this->Cc, $this->Bcc,
			...
			$this->AttachmentFileName, $this->AttachmentContent, $this->EmbeddedImages,$this->pdfAttachmentFileName,$this->pdfAttachmentContent);
...
function ewr_SendEmail($sFrEmail, $sToEmail, $sCcEmail, $sBccEmail, $sSubject, $sMail, $sFormat, $sCharset, $sSmtpSecure = "", $sAttachmentFileName = "", $sAttachmentContent = "", $arImages = array(), $pdfAttachmentFileName="", $pdfAttachmentContent="") {
    ...
    if ($pdfAttachmentFileName <> "" && $pdfAttachmentContent <> "") {
        $mail->AddStringAttachment($pdfAttachmentContent,"Report.pdf",'base64','application/pdf');
    } else if ($pdfAttachmentFileName <> "") {
        $mail->AddAttachment($pdfAttachmentFileName,'Report.pdf','base64','application/pdf');
    }
...
function ewr_LoadEmailCount() {
	...
	if (EWR_EMAIL_WRITE_LOG) {
		...
		if (EWR_EMAIL_WRITE_LOG_TO_DATABASE) {
			...
			$sEmailSql = "SELECT COUNT(*) FROM " . ewr_QuotedName(EWR_EMAIL_LOG_TABLE_NAME) .
...
function ewr_AddEmailLog($sender, $recipient, $subject, $message) {
	...
	if (EWR_EMAIL_WRITE_LOG) {
		...
		if (EWR_EMAIL_WRITE_LOG_TO_DATABASE) {
			global $conn;
			$sEmailSql = "INSERT INTO " . ewr_QuotedName(EWR_EMAIL_LOG_TABLE_NAME) .
...
function ewr_DebugMsg() {
	global $gsDebugMsg;
	return ($gsDebugMsg <> "") ? "<div class=\"ewSpacer\">" . $gsDebugMsg . "</div>" : "";
}
...
function ewr_SetDebugMsg($v, $newline = TRUE) {
	...
	if ($newline && $gsDebugMsg <> "")
		$gsDebugMsg .= "<div class=\"ewSpacer\">&nbsp;</div>";
	$gsDebugMsg .=  $v;
}
...
function ewr_UserAgent() {
	$ua = ewr_ServerVar("HTTP_USER_AGENT");
...
function ewr_IsMobile() {
	global $UserAgent, $EWR_USE_MOBILE_MENU;
	if (!isset($UserAgent))
		$UserAgent = ewr_UserAgent();
	return $EWR_USE_MOBILE_MENU && $UserAgent[2] <> "";
}
...
// - http://ajax.aspnetcdn.com/ajax/jquery.mobile/<version>/ (jquery.mobile)
function ewr_jQueryHost($mobile) {

	// Use files online
	if ($mobile)
		return "http" . (ewr_IsHttps() ? "s" : "") . "://ajax.aspnetcdn.com/ajax/jquery.mobile/%v/";
	else
		return "http" . (ewr_IsHttps() ? "s" : "") . "://ajax.aspnetcdn.com/ajax/jQuery/";
}

// jQuery version
function ewr_jQueryFile($f) {
	$ver = "1.8.2"; // jQuery version (problem with html5 charts)
...
function ewr_GetExtendedFilter(&$fld, $Default = FALSE) {
	...
	} elseif ($FldVal1 == EWR_NULL_VALUE || $FldOpr1 == "IS NULL") {
...
function ewr_DateFilterString($FldOpr, $FldVal, $FldType) {
...
function ewr_CreateFolder($dir, $mode = 0777) {
	...
	if (is_dir($dir) || @mkdir($dir, $mode))
		return TRUE;
	if (!ewr_CreateFolder(dirname($dir), $mode))
		return FALSE;
