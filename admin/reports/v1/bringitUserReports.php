...
//        $sWhereWrk = "`vendor_click_reports`.`np-id` = `np_users`.`user_id` AND `np_users`.`report_daily`=1";
        $sWhereWrk = "`vendor_click_reports`.`np-id` = `np_users`.`user_id` AND `probid_users`.`user_id`= `vendor_click_reports`.`site user`";
...
//        $dailyFilter_from_date =  date("Y-m-d",$currentUnixTime-86400);
        $dailyFilter_from_date =  date("Y-m-d",0);
...
//    $lastUnixTime = 0;
    ...
    $rNpUsers = $conn->Execute("SELECT DISTINCT `vendor_click_reports`.`site user` as `user_id`, `vendor_click_reports`.`np-name` AS `DispFld`,
                                `probid_users`.`email`,  `probid_users`.`name`, `vendor_click_reports`.`user name`, `np_users`.`wkey`
                                FROM `vendor_click_reports`, `np_users`, `probid_users`
                                WHERE $sWhereWrk AND `clickreport`=1 GROUP BY `site user` ORDER BY `site user`");
...
//    $sWhereWrk = "";
    ...
    for($npi = 0, $maxNpi = sizeof($rNpUsers); $npi< $maxNpi; $npi++){
        if(trim($rNpUsers[$npi]["name"]) == "" || trim($rNpUsers[$npi]["user name"]) == "guest shopper" || trim($rNpUsers[$npi]["user name"]) == "guest shoppers")
            continue;

        if( $lastUpdates[$lUi]['type'] == "daily" ){
            $dailyResult = "SELECT SUM(Sales) as sale, SUM(`np-share`)as giveback FROM `vendor_click_reports` WHERE (`site user` = '{$rNpUsers[$npi]["user_id"]}')";
        ...
        $PageContent = file_get_contents("bringitUserReportMailTemplate.html");
        $PageContent = str_replace("{#Organization#}",$rNpUsers[$npi]["name"],$PageContent);
        $PageContent = str_replace("{NP-WKEY}",md5($rNpUsers[$npi]["user_id"]),$PageContent);
        ...
        $rowsContent = "";
        $Page->ResetPager();
//        $Page->Filter = "(`last_update` BETWEEN '$Filter_from_date' AND '$Filter_to_date') AND (`site user` = '{$rNpUsers[$npi]["user_id"]}')";
        $Page->Filter = "(`last_update` BETWEEN '$Filter_from_date' AND '$Filter_to_date') AND (`user name` = '{$rNpUsers[$npi]["user name"]}')";
        $Page->Export = "email";
        ...
        if ($Page->TotalGrps > 0) {
            ...
            $sWhere="(`site user` = '{$rNpUsers[$npi]["user_id"]}')";
...
//                    $rowsContent .='<span'.$Page->user_name->ViewAttributes() .'>'. $rNpUsers[$npi]["user name"] .'</span></td>';
                }
        ...
        if ($lastUpdates[$lUi]['type'] == "daily" && $Page->Sales->SumValue == 0)
            continue;
...
//            $params["recipient"] = "lilian.codreanu@gmail.com";
