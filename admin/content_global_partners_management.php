...
else
{
	...
	if (($_REQUEST['do'] == 'add_global_partner' || $_REQUEST['do'] == 'edit_global_partner') && !isset($_POST['form_save_settings']))
	{
		$item = new item();
		$item->setts = &$setts;
		...
		if ($_REQUEST['do'] == 'edit_global_partner')
		{
			$row_advert = $db->get_sql_row("SELECT * FROM
				" . DB_PREFIX ."partners WHERE advert_id=" . $_REQUEST['advert_id']);
		}
		...
		else if (is_numeric($_POST['file_upload_id'])) /* means we remove a file / media url */
		{
			...
			if ($_REQUEST['do'] == 'edit_global_partner')
			{
				$db->query("UPDATE " . DB_PREFIX . "partners SET advert_img_path='' WHERE advert_id='" . intval($_REQUEST['advert_id']) . "'");	
			}
		...
		$selected_categories = (!empty($row_advert['category_id'])) ? $row_advert['category_id'] : 0;
	...
	if ($_REQUEST['do'] == 'add_global_partner')
	{
		if (isset($_POST['form_save_settings']))
		{
			$template->set('msg_changes_saved', $msg_changes_saved);

			$advert->insert_global_partner($_POST);
		}
		else
		{
			...
			$management_box = $template->process('content_global_partners_management_add_banner.tpl.php');
		}
	}
	else if ($_REQUEST['do'] == 'edit_global_partner')
	{
		if (isset($_POST['form_save_settings']))
		{
			$template->set('msg_changes_saved', $msg_changes_saved);

			$advert->edit_global_partner($_POST, $_POST['advert_id']);
	...
	else if ($_REQUEST['do'] == 'delete_global_partner')
	{
		$template->set('msg_changes_saved', $msg_changes_saved);

		$advert->delete_global_partner($_REQUEST['advert_id']);
	}
	...
	$sql_select_banners = $db->query("SELECT * FROM " . DB_PREFIX . "partners " . $addl_query);
	...
	while ($banner_details = $db->fetch_array($sql_select_banners))
	{
		...
		'	<td align="center"> '.
		'		[ <a href="content_global_partners_management.php?do=edit_global_partner&advert_id=' . $banner_details['advert_id'] . '">' . AMSG_EDIT . '</a> ] &nbsp;'.
		'		[ <a href="content_global_partners_management.php?do=delete_global_partner&advert_id=' . $banner_details['advert_id'] . '" onclick="return confirm(\'' . AMSG_DELETE_CONFIRM . '\');">' . AMSG_DELETE . '</a> ]</td> '.
		'</tr> ';
	...
	$template_output .= $template->process('content_global_partners_management.tpl.php');
