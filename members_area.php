        ...
        if ($section == 'drafts') {
            ...
            if ($nb_items) {
                $sql_select_items = $db->query("SELECT a.*, u.username,
					...
					GROUP BY a.auction_id
					ORDER BY " . $order_field . " " . $order_type . " LIMIT " . $start . ", " . $limit);
                ## PHP Pro Bid v6.00 uses temporary/filesort
...
//				"	oEdit1.REPLACE(\"description_main\");//Specify the id of the textarea here \n" .
    ...
    if ($page == 'contributions') /* BEGIN -> CONTRIBUTIONS PAGE */ {
        if ($section == 'main') {
            ...
            $query .= " ORDER BY funders.created_at DESC";
			
            $campaigns_result = $db->query($query);
    ...
    if ($page == 'campaigns') /* BEGIN -> CAMPAIGNS PAGE */ {
        ...
        if ($section == 'edit') {
            ...
            if ((isset($_REQUEST['operation']) && $_REQUEST['operation'] == 'submit') || ($_POST['ajaxsubmit'] == true) || (isset($_REQUEST['ajaximageupload']))) {
            ...
            if (isset($_REQUEST['ajaximageupload'])) {
                ...
                if (isset ($_FILES["banner"]) && is_uploaded_file($_FILES["banner"]["tmp_name"])) {
                    ...
                    if (in_array($_FILES["banner"]["type"], $allowed_image_mime_types)) {
                ...
                if (isset ($_FILES["banner"]) && is_uploaded_file($_FILES["banner"]["tmp_name"])) {
                    ...
                    $form_submit_msg = array("status" => "success", 'path' => $banner_file_name);
...
//              $members_area_page_content = $template->process('members_area_campaigns.tpl.php');
    ...
    if ((isset($_POST['ajaxsubmit'])) || (isset($_REQUEST['ajaximageupload']))) {
        if (($form_submit_msg == null) || ($form_submit_msg == '')) $form_submit_msg = array("status" => "fail");
        echo json_encode($form_submit_msg);
    } else if (($template_output == '') || ($template_output == null)) {
        $template_output .= "error";
