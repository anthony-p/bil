...
if ($setts['enable_wanted_ads'])
{
	...
	$template->set('header_wanted_ads', header5(GMSG_LOCALWANTED_ADS));
	...
	$categories_header_menu = category_navigator($parent_id, true, true, 'localwanted_ads.php');
	...
	{ 
		...
		$subcategories_content .= '<td class="c1"><img src="themes/' . $setts['default_theme'] . '/img/arrow.gif"></td>'.
			'<td class="c2" width="33%"><a href="' . process_link('localwanted_ads', array('category' => $category_lang[$wanted_cats[$i]['category_id']], 'parent_id' => $wanted_cats[$i]['category_id'])) . '">' . $category_lang[$wanted_cats[$i]['category_id']] . '</a> '.
	...
	$where_query = "WHERE a.active=1 AND a.closed=0 AND a.deleted=0 AND a.npuser_id='$np_userid'" . $addl_where_query;
	...
	$template_output .= $template->process('browse_localwanted_ads.tpl.php');
