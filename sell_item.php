...
$item_details['npuser_id'] = (!empty($item_details['npuser_id'])) ? $item_details['npuser_id'] : $user_details['npuser_id'];
	...
	if ($sale_step == 'finish') /* auction submission page */
	{
		...
		else
		{
			...
			else 
			{
                ...
                if($refresh_id){
                    $points_awarded = $db->get_sql_field("SELECT points_awarded FROM probid_user_activities WHERE activity_id=1", points_awarded);
                    $user_id = $session->value('user_id');
                    $db->query("INSERT INTO probid_user_points (user_id, activity_id, points_awarded, awarded_date)
                              VALUES ('$user_id', '1', '$points_awarded', date(now()))");
	...
	else if ($sale_step == 'main_category') /* choose a main category for your item */
	{
		...
		$choose_category_expl_message = '<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" class="border"> ' .
		'<tr> ' .
