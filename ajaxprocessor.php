...
$mycity=$_GET["city"];
$distancefrom=$_GET["distancefrom"];
...
$sqlnew = "SELECT user_id,username, tax_company_name,( 3959 * acos( cos( radians($Latitude) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians($Longitude) ) + sin( radians($Latitude) ) * sin( radians( lat ) ) ) ) AS distance FROM np_users HAVING distance < $distancefrom and tax_company_name like '%$search_name%' ORDER BY tax_company_name ASC LIMIT 0 , $limitresults";

//original search was ordered by distance below
...
echo "<h3>Select your non profit</h3>";
echo "<select name='tax_company_name' value='' onchange='showUser(this.value)' id='tax_company_name'>Student Name</option>";
echo "<option value=''>Scroll down to select </option>";
...
echo "<br><br><strong>or refine your search</strong><br>";
echo "Distance to search: <input type = 'text' name='distancefrom' value='25' id='distancefrom' size='4'/> miles<br>";
echo "limit results to <input type = 'text' name='limitresults' value='25' id='limitresults' size='4'/> organizations";
echo "<br>and whose name contains:<br>";
echo "<input type = 'text' name='search_name' value='' id='search_name' size='20'/>";
echo "<br><input type='button' value='Search' onClick='doHttpRequest2();'>";
 ...
 $address = $myaddress .",". $mycity .",". $myzipcode;
   ...
   $id = $row["user_id"];
    ...
    if (strcmp($status, "200") == 0) {
      ...
      // failure to geocode
      $geocode_pending = false;
      echo "<h3>No results found. Did you enter a valid zipcode?</h3>";
