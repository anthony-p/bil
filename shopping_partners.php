...
if ($setts['enable_wanted_ads'])
{
	...
	$template->set('header_wanted_ads', header5(GMSG_PARTNER_ADS));
	...
	$categories_header_menu = category_navigator($parent_id, true, true, 'shopping_partners.php');
	...
	{ 
		...
		$subcategories_content .= '<td class="c1"><img src="themes/' . $setts['default_theme'] . '/img/arrow.gif"></td>'.
			'<td class="c2" width="33%"><a href="' . process_link('shopping_partners', array('category' => $category_lang[$wanted_cats[$i]['category_id']], 'parent_id' => $wanted_cats[$i]['category_id'])) . '">' . $category_lang[$wanted_cats[$i]['category_id']] . '</a> '.
	...
	$template_output .= $template->process('browse_shopping_partners.tpl.php');
