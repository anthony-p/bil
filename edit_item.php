...
if ($session->value('membersarea') == 'Active')
{
	$item_details = $db->get_sql_row("SELECT * FROM
		" . DB_PREFIX . "auctions WHERE auction_id='" . intval($_REQUEST['auction_id']) . "' AND owner_id=" . $session->value('user_id'));
}
	...
	if (isset($_REQUEST['form_edit_proceed']))
	{
		...
		else
		{
			...
			else
			{
				...
				$item->edit_auction = true;
				$item->insert($item_details, $session->value('user_id'), $page_handle, $item_details['draft']);
