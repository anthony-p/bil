...
else
{
	...
	else if (!empty($_REQUEST['sc_id']))
	{
		if ($session->value('adminarea') != 'Active')
		{
			$addl_query_sc = " AND (sc.seller_id='" . $session->value('user_id') . "' OR sc.buyer_id='" . $session->value('user_id') . "')";
		}

		$sc_details = $db->get_sql_row("SELECT sc.* FROM " . DB_PREFIX . "shopping_carts sc WHERE 
			sc.sc_id='" . intval($_REQUEST['sc_id']) . "'" . $addl_query_sc);
		...
		if ($sc_details['sc_id'] > 0)
		{
			...
			$msg_details = array('message_handle' => '6', 'sc_id' => $sc_details['sc_id']);

			$message_title = $msg->message_subject($msg_details);
			$message_handle = 6;

			$topic_id = ($sc_details['messaging_topic_id'] > 0) ? $sc_details['messaging_topic_id'] : $topic_id;

			if ($sc_details['seller_id'] == $session->value('user_id'))
			{
				$contact_details = '<tr class="c4"> '.
					'	<td colspan="2">' . MSG_BUYER_CONTACT_DETAILS . '</td> '.
					'</tr>';
				$user_id = $sc_details['buyer_id'];

				$cart->sc_field = 'seller_id';
			}
			else if ($sc_details['buyer_id'] == $session->value('user_id'))
			{
				$contact_details = '<tr class="c4"> '.
					'	<td colspan="2">' . MSG_SELLER_CONTACT_DETAILS . '</td> '.
					'</tr>';
				$user_id = $sc_details['seller_id'];

				/* direct payment box will only appear for the buyer */
				$cart = new shop();
				$cart->setts = &$setts;
				$cart->sc_field = 'buyer_id';
				$cart->sc_value = $session->value('user_id');

				$direct_payment_box = $cart->sc_direct_payment_box($sc_details['sc_id']);
	...
	if (isset($_REQUEST['add_message']) && !$blocked_user)
	{
		...
		else if ($_REQUEST['message_handle'] == 6)
		{
			$topic_id = $msg->new_topic(null, $session->value('user_id'), $user_id, 0, '', $_REQUEST['message'], 6, null, 0, $sc_details['sc_id']);
		}
	...
	while($message_details = $db->fetch_array($sql_select_messages))
	{
		...
		$additional_vars = '&topic_id=' . $topic_id . '&winner_id=' . intval($_REQUEST['winner_id']) . '&bid_id=' . intval($_REQUEST['bid_id']) . '&sc_id=' . intval($_REQUEST['sc_id']);
