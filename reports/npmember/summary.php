...
//	var $SelectLimit = TRUE;
	...
	function cronebar() {
		...
		$this->np_name->GroupingFieldId = 1;
		...
		$this->invoice_date->GroupingFieldId = 2;
		...
		$this->invoice_date->SqlSelect = "SELECT DISTINCT `invoice_date` FROM " . $this->SqlFrom();
		$this->invoice_date->SqlOrderBy = "`invoice_date`";
		$this->invoice_date->FldGroupByType = "";
		$this->invoice_date->FldGroupInt = "0";
		$this->invoice_date->FldGroupSql = "";
		...
		// Chart1
		$this->Chart1 = new crChart('onebar', 'onebar', 'Chart1', 'Chart1', 'np_name', 'points', '', 5, 'SUM', 550, 440);
		$this->Chart1->SqlSelect = "SELECT `np_name`, '', SUM(`points`) FROM ";
		$this->Chart1->SqlGroupBy = "`np_name`";
	...
	function SqlOrderBy() { // Order By
		return "`np_name` ASC, `invoice_date` ASC";
	}
...
// popup fields
<?php $jsdata = ewrpt_GetJsData($onebar->invoice_date, $onebar->invoice_date->FldType); ?>
...
if ($_GET["sv1_np_name"] == $_SESSION[nporgname]){
...
<?php if ($onebar->Export == "") { ?>
&nbsp;&nbsp;<a href="<?php echo $onebar_summary->ExportExcelUrl ?>&sv1_np_name=<?=$_SESSION[nporgname]?>&sv_invoice_date=<?=$_SESSION[sv_onebar_invoice_date]?>"><?php echo $ReportLanguage->Phrase("ExportToExcel") ?></a>
&nbsp;&nbsp;<a href="<?php echo $onebar_summary->ExportWordUrl ?>&sv1_np_name=<?=$_SESSION[nporgname]?>&sv_invoice_date=<?=$_SESSION[sv_onebar_invoice_date]?>"><?php echo $ReportLanguage->Phrase("ExportToWord") ?></a>
...
<!--&nbsp;&nbsp;<a href="summary.php?cmd=reset"><?php echo $ReportLanguage->Phrase("ResetAllFilter") ?></a>-->
<?php } ?>
<?php } ?>
<br /><br />
<?php if ($onebar->Export == "") { ?>
...
<!-- Search form (begin) -->
<form name="fonebarsummaryfilter" id="fonebarsummaryfilter" action="summary.php" class="ewForm" onsubmit="return onebar_summary.ValidateForm(this);">
<table class="ewRptExtFilter">
	<tr>
		<td></td>
		<td>
<input type="hidden" name="so1_np_name" id="so1_np_name" value="="></td>
		<td>
			<table cellspacing="0" class="ewItemTable"><tr>
				<td><span class="phpreportmaker">
<input type="hidden" name="sv1_np_name" id="sv1_np_name" size="30" maxlength="255" value="<?php echo ewrpt_HtmlEncode($onebar->np_name->SearchValue) ?>"<?php echo ($onebar_summary->ClearExtFilter == 'onebar_np_name') ? " class=\"ewInputCleared\"" : "" ?>>
</span></td>
			</tr></table>			
		</td>
	</tr>
...
<?php
	...
	// Build detail SQL
	$sWhere = ewrpt_DetailFilterSQL($onebar->np_name, $onebar->SqlFirstGroupField(), $onebar->np_name->GroupValue());
...
?>
	...
	<tr<?php echo $onebar->RowAttributes(); ?>><td colspan="5"><?php echo $ReportLanguage->Phrase("RptGrandTotal") ?> (<?php echo ewrpt_FormatNumber($onebar_summary->TotCount,0,-2,-2,-2); ?> <?php echo $ReportLanguage->Phrase("RptDtlRec") ?>)</td></tr>
<?php } ?>
	</tfoot>
</table>
</div>
...
<form action="summary.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<!--first page button-->
	...
	<td><a href="summary.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="<?php echo $ReportLanguage->Phrase("PagerFirst") ?>" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="<?php echo $ReportLanguage->Phrase("PagerFirst") ?>" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="summary.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="<?php echo $ReportLanguage->Phrase("PagerPrevious") ?>" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="summary.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="<?php echo $ReportLanguage->Phrase("PagerNext") ?>" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="<?php echo $ReportLanguage->Phrase("PagerNext") ?>" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="summary.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="<?php echo $ReportLanguage->Phrase("PagerLast") ?>" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
ewrpt_SortChartData($onebar->Chart1->Data, 0, "");
...
class cronebar_summary {
		...
		switch ($lvl) {
			case 1:
				return (is_null($onebar->np_name->CurrentValue) && !is_null($onebar->np_name->OldValue)) ||
					...
					($onebar->np_name->GroupValue() <> $onebar->np_name->GroupOldValue());
			case 2:
				return (is_null($onebar->invoice_date->CurrentValue) && !is_null($onebar->invoice_date->OldValue)) ||
					...
					($onebar->invoice_date->GroupValue() <> $onebar->invoice_date->GroupOldValue()) || $this->ChkLvlBreak(1); // Recurse upper level
		}
	}
	...
	function GetGrpRow($opt) {
		...
		if ($opt == 1) { // Get first group
			...
			$onebar->np_name->setDbValue(""); // Init first value
		} else { // Get next group
			$rsgrp->MoveNext();
		}
		if (!$rsgrp->EOF)
			$onebar->np_name->setDbValue($rsgrp->fields[0]);
		if ($rsgrp->EOF) {
	...
	//		$rs->MoveFirst(); // NOTE: no need to move position
		...
		if (!$rs->EOF) {
			...
			if ($opt <> 1) {
				if (is_array($onebar->np_name->GroupDbValues))
					$onebar->np_name->setDbValue(@$onebar->np_name->GroupDbValues[$rs->fields('np_name')]);
				else
					$onebar->np_name->setDbValue(ewrpt_GroupValue($onebar->np_name, $rs->fields('np_name')));
			}
			...
			$onebar->username->setDbValue($rs->fields('username'));
	...
	function SetupPopup() {
		...
		while ($rswrk && !$rswrk->EOF) {
			...
			if (is_null($onebar->invoice_date->CurrentValue)) {
				...
				$onebar->invoice_date->GroupViewValue = ewrpt_DisplayGroupValue($onebar->invoice_date,ewrpt_FormatDateTime($onebar->invoice_date->GroupValue(), 5));
				ewrpt_SetupDistinctValues($onebar->invoice_date->ValueList, $onebar->invoice_date->GroupValue(), $onebar->invoice_date->GroupViewValue, FALSE);
			}
			$rswrk->MoveNext();
		}
		if ($rswrk)
	...
	function RenderRow() {
		...
		if ($onebar->RowType == EWRPT_ROWTYPE_TOTAL) { // Summary row
			...
			$onebar->np_name->CellAttrs["class"] = ($onebar->RowGroupLevel == 1) ? "ewRptGrpSummary1" : "ewRptGrpField1";
			...
			$onebar->invoice_date->GroupViewValue = ewrpt_FormatDateTime($onebar->invoice_date->GroupViewValue, 5);
			$onebar->invoice_date->CellAttrs["class"] = ($onebar->RowGroupLevel == 2) ? "ewRptGrpSummary2" : "ewRptGrpField2";
			...
			$onebar->np_name->CellAttrs["class"] = "ewRptGrpField1";
			$onebar->np_name->GroupViewValue = ewrpt_DisplayGroupValue($onebar->np_name, $onebar->np_name->GroupViewValue);
			if ($onebar->np_name->GroupValue() == $onebar->np_name->GroupOldValue() && !$this->ChkLvlBreak(1))
				$onebar->np_name->GroupViewValue = "&nbsp;";

			// invoice_date
			$onebar->invoice_date->GroupViewValue = $onebar->invoice_date->GroupValue();
			$onebar->invoice_date->GroupViewValue = ewrpt_FormatDateTime($onebar->invoice_date->GroupViewValue, 5);
			$onebar->invoice_date->CellAttrs["class"] = "ewRptGrpField2";
			$onebar->invoice_date->GroupViewValue = ewrpt_DisplayGroupValue($onebar->invoice_date, $onebar->invoice_date->GroupViewValue);
			if ($onebar->invoice_date->GroupValue() == $onebar->invoice_date->GroupOldValue() && !$this->ChkLvlBreak(2))
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			$this->SetSessionFilterValues($onebar->np_name->SearchValue, $onebar->np_name->SearchOperator, $onebar->np_name->SearchCondition, $onebar->np_name->SearchValue2, $onebar->np_name->SearchOperator2, 'np_name');
			...
			// Field np_name
			if ($this->GetFilterValues($onebar->np_name)) {
				$bSetupFilter = TRUE;
		...
		if ($bRestoreSession) {
			...
			// Field np_name
			$this->GetSessionFilterValues($onebar->np_name);
		...
		$this->BuildExtendedFilter($onebar->np_name, $sFilter);
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field np_name
		$this->SetDefaultExtFilter($onebar->np_name, "=", '$sv1_np_name', 'AND', "=", NULL);
		$this->ApplyDefaultExtFilter($onebar->np_name);
	...
	function CheckFilter() {
		...
		if ($this->TextFilterApplied($onebar->np_name))
			return TRUE;
	...
	function ShowFilterList() {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildExtendedFilter($onebar->np_name, $sExtWrk);
		if ($sExtWrk <> "" || $sWrk <> "")
			$sFilterList .= $onebar->np_name->FldCaption() . "<br />";
		if ($sExtWrk <> "")
